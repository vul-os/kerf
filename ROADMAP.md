...
| **5-axis CAM real solver** | 🔮 planned | Replaces the current `5_axis: not_implemented` stub. v1 scope: constant-tilt surface finishing (UV iso-curves + per-point pythonOCC surface normal + tilt-about-tangent + ball-end tip math) + 3+2 indexed (rotate STL to align drive-face normal with +Z). OCL has no native 5-axis primitive — layer on top of pythonOCC `GeomLProp_SLProps`. 8 tasks, ~17 sonnet-agent-days. Deferred: swarf, Sandvik morphing, other kinematic posts, gouge/collision detection. Highest-risk open question: drive-face picker UX. Full design: [docs/plans/5-axis-cam.md](./docs/plans/5-axis-cam.md). |
