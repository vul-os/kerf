...
| ↳ **T2: BRep-lift + `import_brep` worker op** | 📋 next | BRep-bytes → content-hashed lift via pythonOCC `BRepTools_Read`. New worker op `import_brep` in `src/lib/occtWorker.js` (reads BRep blob from storage → `TopoDS_Shape`), plus `read_only: true` skip in the dispatcher. Deps: T1. |
