...
| kerf-cad-core / aero | Thin-airfoil Cl/Cm | **Wired** | `airfoilPolarBridge.ts` + `AirfoilPolarPlot.tsx` wired (2026-05-24) |
...
| Package | Capability | Frontend Status | Notes |
|---------|-----------|-----------------|-------|
| kerf-api (new) | Tsiolkovsky Δv | **Route exists, no UI** | `POST /api/aero/propulsion/tsiolkovsky` — shipped this sweep; bridge exists (`aeroPropulsionBridge.ts`) |
...
| kerf-fem | FEM solver | Partial | `femDisplacement.ts` renders displacement; backend route exists in kerf-fem plugin |
...
| Package | Capability | Frontend Status | Notes |
|---------|-----------|-----------------|-------|
| kerf-electronics | PCB autoroute (freerouting) | **Wired** | `POST /api/electronics/autoroute`; `pcbRouting.ts` + shove router UI |
| kerf-electronics | Copper pour | **Wired** | `copperPour.ts` frontend; pour route in plugin |
| kerf-electronics | SPICE simulation | Partial | `circuitToSpice.ts`; backend `routes_spice.py` exists; no results panel |
...
| kerf-electronics | DRC (design-rule check) | **Wired** | `pcbDRC.ts` frontend-only; no backend route needed |
| kerf-electronics | ERC (electrical rule check) | **Wired** | `erc.ts` frontend-only |
...
| kerf-electronics | PCB schematic capture | **Wired** | tscircuit circuit runner; `circuitRunner.ts` |
| kerf-electronics | PCB layout (tscircuit) | **Wired** | tscircuit layout engine; `circuitWorker.ts` |
...
| Package | Capability | Frontend Status | Notes |
|---------|-----------|-----------------|-------|
| kerf-cad-core | OCCT B-rep kernel (Phase 2–3) | **Wired** | `occtWorker.ts` + `occtRunner.ts`; full Pad/Pocket/Fillet pipeline |
| kerf-cad-core | NURBS surfacing (Phase 4) | Partial | `surfacing.py`; frontend `subd.ts`; NURBS long-tail |
| kerf-cad-core | Sketch solver (PlaneGCS) | **Wired** | `sketchSolver.ts` + WASM; full constraint solving |
...
| kerf-render | Path-tracing render | **Wired** | `heroShot.ts` + `heroShotBrowserPT.ts`; render dropdown |
| kerf-cam | CAM toolpath generation | **Wired** | `kerf-cam` plugin; `CAMView.tsx` + `LayeredCAMView` wired in Editor (2026-05-24) |
| kerf-slicing | FDM slicing (Cura) | **Wired** | `POST /run-print-slice`; `PrintSliceView.tsx` imported and rendered in Editor (2026-05-24) |
| kerf-imports | FreeCAD import | Route exists, no UI | `POST /import-freecad-project`; drag-and-drop import UI |
| kerf-imports | Rhino 3dm import | Route exists, no UI | `rhino3dm_route.py`; `rhino3dm.ts` frontend |
...
| kerf-bim | BIM IFC | **Wired** | BIM plugin route; `BIMView.tsx` wired in Editor (2026-05-24) |
| kerf-cad-core / arch | Architectural elements | Partial | `curtainWall.ts`, `railings.ts`, `stairs.ts` frontend; no route |
...
| Package | Capability | Frontend Status | Notes |
|---------|-----------|-----------------|-------|
| kerf-wiring | WireViz harness diagram | **Wired** | `POST /run-wireviz`; `WiringView.tsx` wired in Editor (2026-05-24) |
| kerf-plc | PLC IEC-61131 lint | **Wired** | `POST /lint-plc`; `PLCView.tsx` imported and rendered in Editor (2026-05-24) |
...
| kerf-cad-core / dfm | DFM checks | Partial | `dfmOverlay.ts` frontend; no backend DFM route |
...
| kerf-parts | Parts + distributor search | **Wired** | Library/BOM distributor routes; `BOMTable.tsx` |
...
| Package | Capability | Frontend Status | Notes |
|---------|-----------|-----------------|-------|
| kerf-topo | Topology optimisation | **Wired** | `/run-topo`; `TopoView.tsx` imported and rendered in Editor (2026-05-24) |
