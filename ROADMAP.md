...
| **Rhino parity: SubD modelling** | 🔮 planned | T-splines / Catmull-Clark subdivision surfaces. Open-source path: OpenSubdiv (Pixar, Apache-2.0) on top of OCCT meshes — produces watertight subdivided BRep that round-trips through booleans. `.subd` file kind; `feature_subd_from_quad_mesh`, `feature_subd_extrude_face`, `feature_subd_split_edge` ops. Mid-large lift — separate from NURBS Phase 4. Plan-doc TBD. |
| **Rhino parity: Grasshopper-style node graph** | 🔮 planned | Visual node-graph definition language. `.gh` (`.graph`?) file kind compiles to feature trees. Nodes: parameters, math, transforms, list ops, feature-tree builders. Same Cmd+Z + revision history applies. Likely React Flow + a small visit/compile pass. Mid lift. Plan-doc TBD. |
| **Rhino parity: Quad remesher** | 🔮 planned | Equivalent of Rhino's QuadRemesh / ZRemesher. Open-source path: Instant Meshes (MIT, ~10k LOC C++) — wasm or native subprocess. `feature_quad_remesh` op on a triangle mesh → quad-dominant mesh. Useful for SubD prep + downstream FEM. Plug-in territory. |
| **Sketcher: construction + external geometry** | 🔮 planned | Project edges from existing solids/faces into the sketch as construction geometry (`isConstruction: true`), so downstream sketch geometry can constrain to them. Mirrors FreeCAD's "External Geometry" tool. New sketch-entity kind `external_edge_ref: { feature_id, edge_id }`; planegcs solver treats them as fixed. |
| **Sketcher: B-spline + Bezier curves** | 🔮 planned | Add `bspline` + `bezier` entity kinds to `.sketch`. planegcs already supports constraint on control polygons; UI: draw via control-point clicks; Tangent / G1 / G2 continuity constraints between adjacent segments. FreeCAD-Sketcher parity gap. |
| **Sketcher: carbon copy + symmetry over construction line** | 🔮 planned | "Carbon copy" clones an existing sketch with optional transform — useful for left/right symmetric parts. "Symmetry over construction line" mirrors a sub-selection across an arbitrary construction line (current Horizontal/Vertical symmetry is axis-only). |
| **Revit parity: phasing** | 🔮 planned | Existing / demo / new construction phases on `.bim` elements. Per-element `phase_created` + `phase_demolished` slots; view filters can hide based on active phase. IfcOpenShell exports already carry `IfcPhase` — this just plumbs the slot. |
| **Revit parity: view filters + overrides** | 🔮 planned | Per-view element visibility + graphic overrides driven by a filter DSL (`category=wall AND fire_rating>=120` style). `.view.json` schema grows `filters: [{ rule, override }]`. Renderer applies overrides in `BIMView.jsx`. |
