...
function makeBoard(overrides = {}): { type: string; width: number; height: number; [key: string]: unknown } {
...
describe('resolveSheetPin', () => {
  it('resolves matching pin → hierarchical label pair', () => {
    const parent = addSubSheet(makeBoard(), {
      name: 'PSU',
      file_id: 'f-psu',
  ...
  it('returns null when pin does not exist', () => {
    const parent = addSubSheet(makeBoard(), { name: 'PSU', file_id: 'f-psu', pins: [] })
    const sid = parent.sub_sheets[0].id
...
describe('flattenHierarchy', () => {
  it('merges parent pin net with child hier label net', () => {
    const top = addSubSheet(makeBoard(), {
      name: 'PSU',
      file_id: 'f-psu',
  ...
  it('handles missing child gracefully (skips that sheet)', () => {
    const top = addSubSheet(makeBoard(), { name: 'Missing', file_id: 'f-missing' })
...
describe('validateHierarchy', () => {
  ...
  it('reports orphaned hierarchical_label (label with no matching pin)', () => {
    const top = addSubSheet(makeBoard(), { name: 'PSU', file_id: 'f-psu', pins: [] })
    const sid = top.sub_sheets[0].id
  ...
  it('reports global label name collision on same sheet', () => {
    const board = addGlobalLabel(makeBoard(), 'GND', 'net-gnd-a')
