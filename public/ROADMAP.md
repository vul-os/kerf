...
| **Exact arc-length-preserving offset** | Klass 1980 / Maekawa 1999 §3 — offset curve whose arc length matches the original exactly (stylised seam/character lines); `offset_curve_arclength_preserving` + LLM tool `nurbs_offset_arclength_preserving` | ✅ shipped |
...
| **Volume-weighted centroid + inertia for non-uniform density** | Monte Carlo volume integration (Mortenson §11.5); `functionally_graded_centroid` for linear-z / shell-dense / radial FGM patterns; LLM tool `brep_centroid_density_field`; 22 tests green. | ✅ shipped |
| **BREP-EDGE-CONVEX-CONCAVE-CLASSIFY** | Dihedral-angle classification of every interior edge as convex / concave / tangential; `classify_edges` + `brep_classify_edges` LLM tool (Hoffmann 1989 §5.3; Mantyla 1988 §7.4) | ✅ shipped |
...
**Civil / infrastructure (distinct engines):** plan-and-profile sheet engine · IFC-4.3-infra I/O · bridge/tunnel · mining · marine/dredging · rail signaling. *(CRS engine, horizontal/vertical alignment, corridor, earthworks, geotechnical, LandXML 1.2 I/O, pressurized water-distribution networks (Todini GGA + Hazen-Williams/Darcy-Weisbach), gravity sewer (Manning), storm rational method + HDS-5 culverts, rational drainage method + composite watershed (FHWA HEC-22) already shipped.)*
