...
try:
    ...
    def _parse_curve_ns(entry: dict):
        ...
        if "p0" in entry and "p1" in entry:
            p0 = _np.asarray(entry["p0"], dtype=float).ravel()
            p1 = _np.asarray(entry["p1"], dtype=float).ravel()
            if p0.shape[0] < 3:
                p0 = _np.concatenate([p0, _np.zeros(3 - p0.shape[0])])
            if p1.shape[0] < 3:
                p1 = _np.concatenate([p1, _np.zeros(3 - p1.shape[0])])
            return make_line_nurbs(p0[:3], p1[:3])

        if "points" in entry:
            pts = _np.asarray(entry["points"], dtype=float)
            ...
            n = pts.shape[0]
            degree = 1
            knots = _np.zeros(n + degree + 1)
            knots[:degree + 1] = 0.0
            knots[-(degree + 1):] = 1.0
            if n > 2:
                inner = _np.linspace(0.0, 1.0, n)[1:-1]
    ...
    _nsided_patch_spec = ToolSpec(
        ...
        description=(
            ...
            "    {p0: [x,y,z], p1: [x,y,z]} line-segment objects.\n"
            "internal_curves : optional list of cross-curves for shape control.\n"
    ...
    async def _run_nurbs_n_sided_patch(ctx, args: bytes) -> str:
        try:
            a = _json.loads(args)
        ...
        if not boundary_raw:
            return _json.dumps({"ok": False, "reason": "'boundary_curves' is required"})

        if not isinstance(boundary_raw, list) or len(boundary_raw) < 3:
            return _json.dumps({"ok": False, "reason": "'boundary_curves' must be a list of >= 3 curves"})

        try:
            boundary_curves = [_parse_curve_ns(entry) for entry in boundary_raw]
        except Exception as exc:
            return _json.dumps({"ok": False, "reason": f"parsing boundary_curves: {exc}"})

        internal_curves = None
        if a.get("internal_curves"):
            try:
                internal_curves = [_parse_curve_ns(entry) for entry in a["internal_curves"]]
            except Exception as exc:
                return _json.dumps({"ok": False, "reason": f"parsing internal_curves: {exc}"})

        method = a.get("method", "coons_gregory")
        tol = float(a.get("tol", 1e-6))
        grid_n = int(a.get("grid_n", 24))
        grid_n = max(6, min(64, grid_n))
        compute_fairness_flag = bool(a.get("compute_fairness", False))
        ...
        try:
            ...
            return _json.dumps({"ok": False, "reason": str(exc)})
        except Exception as exc:
            return _json.dumps({"ok": False, "reason": f"fit_network_patch error: {exc}"})
