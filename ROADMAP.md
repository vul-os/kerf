...
| ↳ **T2: `opToSolid` worker handler + dispatch** | 📋 next | Add `opToSolid` to `src/lib/occtWorker.js`; wire `case 'to_solid'` into **both** `evaluateTree` and `evaluateToFinalShape` (dormant-node-bug template: `opBossWithDraft`). Introduce a `bodyMap: { [feature_id]: TopoDS_Shape }` so `to_solid` / `boolean` can lookup arbitrary upstream nodes. Deps: T1. |
...
| ↳ **T3: Sketch translator (Sketcher → `.sketch`)** | 📋 next | Translate FreeCAD `Geometry` + `Constraints` property lists → Kerf `.sketch` JSON. Hand-curated constraint mapping table (Coincident, Horizontal, Distance{X,Y}, Angle, Tangent, Equal, PointOnObject, Symmetric, Block, Diameter). Drops for Snell's-Law / Weight / InternalAlignment with warnings. New `freecad/sketch.py`. Deps: T1. |
| ↳ **T4: PartDesign feature-tree metadata capture** | 📋 next | Walk PartDesign features, emit `.feature` JSON: `import_brep` node first + read-only metadata nodes per Pad/Pocket/Fillet/Chamfer/Pattern with `freecad_ref` provenance field. New `freecad/features.py`. Deps: T1, T2, T3. |
| ↳ **T5: Multi-Body `.FCStd` → `.assembly`** | 📋 next | Detect multi-Body documents, emit `.assembly` with one Component per Body; `Placement` properties → 4×4 transforms. New `freecad/assembly.py`. Deps: T1, T4. |
| ↳ **T6: Pyworker route `POST /import-freecad-project`** | 📋 next | Stand up FastAPI route in `freecad.py` (rewrites the existing stub); runs T1+T3+T4+T5 and returns the structured `{created_files, stats, warnings}` response. Deps: T1, T3, T4, T5. |
| ↳ **T7: LLM tool `import_freecad_project`** | 📋 next | New `packages/kerf-imports/src/kerf_imports/tools/import_freecad.py`. Same shape as `import_3dm`: fetch blob, call pyworker, walk response, insert files in PG. `mode: "project" \| "library"` flag. Deps: T6. |
| ↳ **T8: Integration test + `.FCStd` fixtures** | 📋 next | Five `.FCStd` fixtures (`single_pad`, `pad_and_pocket`, `two_bodies`, `sketch_constraints`, `unsupported_constraints`) built once via `freecadcmd` script + committed. End-to-end test uploads each + asserts project file tree + geometry hash. Deps: T7. |
