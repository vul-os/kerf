...
// equations.js doesn't export types, so the row/doc shapes are declared here
// to match what parseEquations/serializeEquations/evaluateEquations produce.
interface EquationParam {
  name: string
  expr: string
  unit?: string
  comment?: string
...
interface EquationParseError {
  paramIndex: number
  message: string
}

interface EquationsDoc {
  version: number
  params: EquationParam[]
  errors: EquationParseError[]
}

interface EquationEvalError {
  paramIndex: number
  name: string
  message: string
}
...
export default function EquationsEditor() {
  ...
  //
  // Reset happens during render (not in an effect) so it lands in the same
  // commit as the `currentFileContent` change instead of triggering an extra
  // render pass — the "adjusting state when a prop changes" pattern from
  // https://react.dev/learn/you-might-not-need-an-effect.
  const [doc, setDoc] = useState<EquationsDoc>(() => parseEquations(currentFileContent || ''))
  const [syncedContent, setSyncedContent] = useState(currentFileContent)
  if (currentFileContent !== syncedContent) {
    setSyncedContent(currentFileContent)
  ...
  function commit(next: EquationsDoc) {
    setDoc(next)
  ...
  const errorByIndex = useMemo(() => {
    const m = new Map<number, EquationEvalError>()
    for (const e of evaluated.errors) m.set(e.paramIndex, e)
    return m
  }, [evaluated.errors])

  function updateRow(i: number, patch: Partial<EquationParam>) {
...
function StatusBadge({ errors, resolved }: { errors: number; resolved: boolean }) {
  if (errors > 0) {
    return (
