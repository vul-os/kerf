/**
 * heroRender.ts — One-click photo-real hero render for jewellery.
 *
 * Exports:
 ...
 *
 * Typing note: like turntableRender.ts, every Three.js object here is
 * deliberately duck-typed (HeroScene/HeroLight/HeroCamera below) rather than
 * imported as real `THREE.*` types — this module does NOT import `three` at
 * all, by design (see `_getThree()`), so its tests can run in jsdom without a
 * GPU. HeroCamera/renderer reuse turntableRender.ts's OrbitCamera/OrbitRenderer
 * since renderHeroSet hands its camera/renderer straight to recordTurntable.
 */

import { recordTurntable } from './turntableRender.js'
import type { OrbitCamera, OrbitRenderer, OrbitTarget } from './turntableRender.js'

// ── Duck-typed Three.js surfaces ─────────────────────────────────────────────

/** The minimal light shape this module reads/writes — real THREE lights and the
 *  test stubs (fakeThree) both satisfy it. */
...
/** The minimal scene shape this module touches — `.children` plus optional
 *  `.add`/`.remove`, matching a real THREE.Scene structurally. */
export interface HeroScene {
  children?: unknown[]
  add?(obj: HeroLight): void
  remove?(obj: HeroLight): void
}

/** {@link OrbitCamera} plus `fov`, which renderHeroSet reads/writes for the
 *  macro-detail shot's shallow-DOF simulation (OrbitCamera doesn't need it). */
export interface HeroCamera extends OrbitCamera {
  fov?: number
}

export interface RenderHeroSetOpts {
  width?: number
  height?: number
  target?: OrbitTarget
  radius?: number
...
export interface RenderHeroSetResult {
  stills: string[]
  turntable: string[]
}

export interface ComposeContactSheetOpts {
  cellSize?: number
  gap?: number
  background?: string
}
 ...
 *
 * @returns Saved light objects that were in the scene before this call.
 */
export function applyJewelryLighting(scene: HeroScene | null | undefined): HeroLight[] {
  ...
  const saved = (scene.children || []).filter((c): c is HeroLight => _isLight(c))
 ...
 */
export function restorePrevLighting(scene: HeroScene | null | undefined, savedLights: HeroLight[] | null | undefined): void {
  if (!scene) throw new Error('scene is required for restorePrevLighting')

  // Remove any jewelry lights that are still in the scene.
  const toRemove = (scene.children || []).filter((c): c is HeroLight => {
    if (!_isLight(c)) return false
    return !!c.userData?.['_heroJewelry']
  })
 ...
 */
export async function renderHeroSet(
  scene: HeroScene | null | undefined,
  camera: HeroCamera | null | undefined,
  renderer: OrbitRenderer | null | undefined,
  opts: RenderHeroSetOpts = {},
): Promise<RenderHeroSetResult> {
 ...
 *
 * @param images  Array of at least 4 PNG data-URL strings.
 * @returns PNG data-URL of the composed 2×2 sheet.
 */
export function composeContactSheet(images: string[] | null | undefined, opts: ComposeContactSheetOpts = {}): string {
 ...
 *
 * @param image  PNG or JPEG data-URL to crop.
 * @returns Map of platform → data-URL.
 */
export async function socialMediaCrops(
  image: unknown,
  platforms: string[] = ['instagram_post', 'instagram_story', 'x_card'],
): Promise<Record<string, string>> {
  if (!image || typeof image !== 'string') {
    throw new Error('image is required for socialMediaCrops')
  }

  const RATIOS: Record<string, CropRatio> = {
...
/** THREE namespace surface this module needs, accessed only via globalThis
 *  (see {@link _getThree}) — never imported, so tests can run without a GPU. */
interface ThreeLikeNamespace {
  DirectionalLight: new (color: number, intensity: number) => HeroLight & {
    position: NonNullable<HeroLight['position']>
  }
  AmbientLight: new (color: number, intensity: number) => HeroLight
}
 ...
 */
function _buildJewelryLights(): HeroLight[] {
 ...
 */
function _isLight(obj: unknown): obj is HeroLight {
  if (!obj) return false
  const l = obj as HeroLight
  ...
  const name = l.constructor ? l.constructor.name : ''
  return !!name && name.endsWith('Light')
}
 ...
 */
function _getThree(): ThreeLikeNamespace | null {
  const g = globalThis as typeof globalThis & { THREE?: ThreeLikeNamespace }
  if (typeof g !== 'undefined' && g.THREE) return g.THREE
  return null
}
 ...
 *
 * @param elevation  radians above XZ plane
 * @param azimuth    radians around Y-axis
 */
function _placeCamera(camera: HeroCamera, target: OrbitTarget, radius: number, elevation: number, azimuth: number): void {
  const tx = target.x ?? 0
  const ty = target.y ?? 0
  const tz = target.z ?? 0
 ...
 */
function _centredCropDims(srcW: number, srcH: number, rw: number, rh: number): number {
  const byWidth = srcW
 ...
 * environments.
 */
function _loadImage(src: string): Promise<ImageDims> {
