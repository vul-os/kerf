...
| ↳ **T1: `.FCStd` parser plumbing** | 📋 next | Pure-Python `.FCStd` → in-memory `FCStdDocument` representation: zip + `Document.xml` walk + `FileIncluded` byte loader for BRep blobs. New `packages/kerf-imports/src/kerf_imports/freecad/parser.py`. No FreeCAD dep — pythonOCC only. Unit tests against `single_pad.FCStd` + `sketch_constraints.FCStd` fixtures. Deps: none. |
