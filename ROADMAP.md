...
| **Slicing: 3D-print G-code (FDM / SLA)** | 🔮 planned | Mesh → printable G-code with perimeters, infill, supports, retraction. Open-source path: wrap [CuraEngine](https://github.com/Ultimaker/CuraEngine) or [PrusaSlicer](https://github.com/prusa3d/PrusaSlicer) as a subprocess. Both are **AGPLv3** — same shape as WireViz: subprocess boundary keeps the hosted service compatible; OSS local install needs the user to opt in via an optional extra. New `.print` file kind (printer + filament + slice profile), `POST /run-slice` pyworker route, `PrintSliceView.jsx` (layer preview + per-layer G-code). Large lift — credible v1 is ~10-15 sonnet-days. |
