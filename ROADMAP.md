...
| **Electrical: wiring + harness diagrams (`.wiring`)** | 🔮 planned | Text DSL → SVG/PDF wiring diagrams. Adopt [WireViz](https://github.com/wireviz/WireViz) (Python, MIT) — YAML schema describing connectors, cables, conductor counts, color codes. New `.wiring` file kind + `WiringView.jsx` (just `<img>` the SVG). Targets cable harnesses + control panels + automotive looms. Pyworker `POST /run-wireviz` wraps the lib. Small lift, high industrial demand. |
