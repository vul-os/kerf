...
| **P0-5** | **Large-assembly performance ceiling** — measured budget + LOD / lazy-load for 1000s of parts. Automotive full-vehicle DMU is the extreme case. | First credibility block for automotive and large mechanical. | 🚧 in flight — `assembly/perf.py`: LOD planner + lazy-load ordering + performance harness shipped; viewport integration + real-part catalogue pending; LOD wireframe box-proxy InstancedMesh created (`_createInstBoxProxy`); box-proxy raycaster interaction ✅ (Wave 4J: `_lodBoxProxyFor` pick + hover + selection) — `kerf_cad_core/assembly/perf.py`, `src/components/Renderer.jsx` |
