"""
Compatibility shims for running kerf_gdnt outside of the legacy backend.

When kerf_gdnt is installed as a plugin (and `kerf_chat.tools.registry` /
`kerf_core.utils.context` are not on the path) these thin replacements are used
so the module still imports cleanly.  The real implementations are provided by
kerf-core at runtime — these shims are only needed during unit-testing or
standalone import.
"""
