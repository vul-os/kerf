...
| **Sketcher: Bezier curves** | 🔮 planned | Add `bezier` entity kind to `.sketch` (cubic B-spline already shipped — see "Sketcher v2 improvements" above). UI: draw via control-point clicks; Tangent / G1 / G2 continuity constraints between adjacent segments. FreeCAD-Sketcher parity gap. |
...
| **Slicing: plane-section / cross-section** | 🔮 planned | CAD-side feature op. New `feature_section` wraps `BRepAlgoAPI_Section` (gating class already in the NURBS Phase 4 C1 binding probe) — takes a target solid + `gp_Pln` plane → returns the intersection edges as a 2D outline. Section-plane gumball in the viewport (drag to reposition, snap to face/edge). Result stored as a `.section` file kind (geom2-style wire) so it can be dimensioned, exported to DXF, or chained into `feature_pad`. Small lift — OCCT already does the work. |
