...
| ↳ **`feature_boss_with_draft`** | 📋 next | Pad + draft in one node. OCCT: `BRepPrimAPI_MakePrism` then per-side-face `BRepOffsetAPI_DraftAngle` with neutral plane = sketch plane. Schema: `{sketch_path, height, direction, draft_angle_deg ∈ [-30,30], draft_direction}`. New worker handler `opBossWithDraft` + `walkSideFaces` helper. Frontend: new entry in `FEATURE_KINDS` slotted into the `sketch` category. ≈1 sonnet-day. |
| ↳ **`feature_cut_from_sketch`** | 📋 next | Subtract a sketched region from any planar face of a target body, normal to that face (unlike `pocket` which cuts normal to the sketch plane). OCCT: `faceById` + `faceFrame` → `placeFaceOnPlane(face-anchored)` → `BRepPrimAPI_MakePrism(-normal*depth)` → `BRepAlgoAPI_Cut_3`. Schema: `{target_id, target_face_id, sketch_path, depth, reverse}`. Shares `push_pull`'s face-id stability caveat. ≈1 sonnet-day. |
| ↳ **`feature_hole_pattern_from_sketch`** | 📋 next | New parametric `hole_pattern` op: one node references a sketch carrying point entities, worker iterates points at eval time and runs N cylinder cuts on the body. Refactors `opHole`'s cylinder block into shared `cutCylinderAtPoint` helper. Schema: `{target_id, sketch_path, diameter, depth}`. Non-point entities silently ignored (so users can mix in construction circles). ≈1 sonnet-day. |
| ↳ **Loft `symmetric` flag (mid-plane)** | 📋 next | Extend `feature_loft` with `symmetric: true` — `opLoft` mirrors both profiles across the mid-plane and feeds `[p1, p2, p2', p1']` to `BRepOffsetAPI_ThruSections` for a thin-walled symmetric body. Limited to exactly 2 input profiles; non-parallel sketch planes → `BAD_ARGS`. No new tool, no new op — flag on existing infra. ≈0.5 sonnet-days. |
| ↳ **Sweep1 `mode` flag (tangent-locked)** | 📋 next | Expose end-to-end the `corrected_frenet` mode the worker already supports. Add `mode: "auto"\|"frenet"\|"corrected_frenet"` to the `feature_sweep1` backend tool's input schema (matching `feature_sweep2`); confirm FeatureView's `sweep1` kind already renders the select (it does). Vitest sweep over a coil path to verify section orientation tracks `T̂(s)`. ≈0.5 sonnet-days. |
...
| ↳ Implement `extrude_sketch_to_jscad` LLM tool | 📋 next | New Python tool in `kerf-cad-core` (or sibling `jscad_scaffold.py`). Validates the sketch exists + parses to ≥ 1 closed loop, then emits a canonical `.jscad` source per op (`extrude_linear` / `extrude_rotate` / `sweep_along_path`). Pytest covers happy path, missing sketch, malformed sketch, target collision. |
| ↳ Surface sketch-import errors in JSCAD viewport | 📋 next | `resolveSketchImports` in `src/lib/jscadRunner.js` currently swallows missing files with a `console.warn` + empty Geom2. Promote unresolved paths to thrown errors so `runJscad` reports them via `partsError`. |
| ↳ Reactive re-eval when an imported sketch changes | 📋 next | Wire `updateSketch` (and the post-LLM-tool refresh path) in `src/store/workspace.js` to re-run JSCAD when the currently-open `.jscad` imports the mutated sketch. Cross-file graph invalidation deferred. |
| ↳ "Build 3D" SketchView affordance + file-tree backlink | 📋 next | SketchView toolbar dropdown — Extrude → / Revolve → — invokes `extrude_sketch_to_jscad` and opens the new file. File-tree row renders a subtle "← <sketch-name>" chip on `.jscad` files that import a sketch. |
