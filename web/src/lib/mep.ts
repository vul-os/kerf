 ...
 */
export function computePressureDrop(route, fluid_props: { density_kg_m3?: number; velocity_m_s?: number; viscosity_Pa_s?: number } = {}) {
...
export function connectEndpoints(route, start_id, end_id, obstacles = [], gridSize_mm = 300) {
  ...
  for (let i = 0; i < polyline.length - 1; i++) {
    ...
    const seg: { id: string; from: unknown; to: unknown; kind: string; elbow_radius_mm?: number } = {
