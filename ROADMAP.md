...
| **NURBS surfacing Phase 4 — full Capability 1 (Robust NURBS-NURBS booleans)** | 🚧 in flight | Surface-direct boolean path that accepts Face/Shell operands without a solid round-trip. Design doc: [docs/plans/nurbs-phase-4-full.md](./docs/plans/nurbs-phase-4-full.md) § Capability 1. |
...
| ↳ **C1-T3: `feature_surface_boolean` Python tool** | ✅ shipped | `feature_surface_boolean_spec` + `run_feature_surface_boolean` added to `packages/kerf-cad-core/src/kerf_cad_core/surfacing.py`. Fields: `file_id`, `target_a_id`, `target_b_id`, `kind` (cut/fuse/common), `fuzziness` (optional, default absent = 1e-4 in worker). Node op = `"surface_boolean"`. Auto-id prefix `"surface_boolean-"`. Fuzziness zero/negative rejected with BAD_ARGS. Registered via `@register` decorator (same `_TOOL_MODULES` path as `feature_boolean`). LLM doc page: `packages/kerf-chat/llm_docs/feature_surface_boolean.md` (schema, worked examples, binding-coverage honesty, error table). 15 pytest cases in `packages/kerf-cad-core/tests/test_feature_surface_boolean.py`. |
| ↳ **C1-T4 through C1-T10** | 🔮 planned | Inspector entry (C1-T5), FeatureView affordance, WASM integration test (C1-T8, three end-to-end scenarios), SetFuzzyValue fallback path if MISSING (C1-T9), custom WASM rebuild escalation if Cut_3 rejects non-solid operands (C1-T10). Full scope in plan doc. |
...
| **Slicing: CNC layered slicing** | 🔮 planned | Stacked plane-sections at fixed Z heights for layered milling / waterjet / laser-cut-and-stack workflows. Wraps the cross-section feature with a layer count + Z-step config; emits a `.cam.layered` file kind. Each layer is a 2D contour fed into the existing CAM contour op. Post-processor adds Z-step retracts between layers. Builds on the cross-section feature row above. |
