...
function sanitizeEntry(raw: Record<string, unknown> | null | undefined): AppearanceEntry | null {
 ...
 */
export function parseAppearance(content: string): Record<string, AppearanceEntry> {
  ...
  for (const line of content.split('\n', SCAN_LINES)) {
    ...
    try {
      const obj: unknown = JSON.parse(trimmed.slice(MARKER.length).trim())
      if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return {}
      const out: Record<string, AppearanceEntry> = {}
      for (const [partId, entry] of Object.entries(obj as Record<string, unknown>)) {
        const clean = sanitizeEntry(entry as Record<string, unknown>)
 ...
 */
export function writeAppearance(content: string, appearance: Record<string, Record<string, unknown>>): string {
  ...
  const cleaned: Record<string, AppearanceEntry> = {}
