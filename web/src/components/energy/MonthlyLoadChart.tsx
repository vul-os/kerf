...
type NumericField = 'heating_kWh' | 'cooling_kWh' | 'lighting_kWh' | 'equipment_kWh'

const SERIES: { key: NumericField; label: string; color: string }[] = [
...
export interface MonthlyLoadDatum {
  month?: string
  heating_kWh?: number
  cooling_kWh?: number
  lighting_kWh?: number
...
export default function MonthlyLoadChart({
  ...
  const months: MonthEntry[] = useMemo(() => {
    if (!Array.isArray(data) || data.length === 0) {
      return MONTHS.map((m) => ({ month: m, heating_kWh: 0, cooling_kWh: 0, lighting_kWh: 0, equipment_kWh: 0 }))
    }
