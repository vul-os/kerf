...
hosted-tier code under `packages/kerf-{billing,cloud}/` and `src/cloud/`,
...
| P1-2 | Jewelry | **Jewelry toolkit** — gem/stone library, prong/head placement, pavé/channel auto-array on a surface, bezel/cutter generation, ring sizer, metal weight + casting cost. *The only gap needing real new breadth — biggest build of the five* | 🚧 in flight — Python tools + LLM docs + FeatureView inspectors + UI + tests shipped across `kerf_cad_core.jewelry.{gemstones,gem_seat,settings,ring,metal_cost}` (7 cuts, prong/bezel/channel/pavé, US/UK/EU/JP sizer + 7 shank profiles, casting-cost panel; `.gem` kind migration 060). **Remaining: OCCT JS worker `op*` handlers** (`opGemstone/opGemSeat/opJewelryProngHead/opJewelryBezel/opJewelryChannel/opJewelryPave/opRingShank`) — node specs are written but geometry won't render until wired in `src/lib/occtWorker.js`. Also deferred: fancy cuts (radiant/asscher/trillion/heart), coloured-stone density correction. |
...
| **Billing: free / paid two-bucket model + wallet top-up** | ✅ shipped | Free tier: monthly chat-token quota redeemable only on cheap-tier models (Sonnet 4.7 / Gemini 3 Flash / DeepSeek / MiniMax). Paid: at-cost pass-through (no markup) against a wallet balance — subscription tiers include monthly credit allowance; users top-up the wallet via Paystack for overage, same shape as Anthropic Console / OpenAI billing. Per-API-token daily cap closes stolen-credential blast radius. (BYO-key plumbing landed dormant in `kerf-billing/buckets.py`; no UI surface — retained for future privacy/compliance use cases.) |
| **Cloud deployment: fly.io + Tigris** | ✅ shipped | `fly.toml` (primary JNB, secondary FRA) + `fly.worker.toml` for the worker fleet. Storage backend wired to Tigris (fly.io S3-compatible) — picked over Cloudflare R2 for fly anycast locality + zero in-fly egress. Multi-stage Dockerfile embeds the compiled Vite SPA at `/app/dist` and FastAPI serves it via `_mount_frontend` with SPA fallback for React Router routes. Deployment guides: `deployment/fly.md`, `deployment/tigris.md`. |
...
| **Plugin architecture + monorepo packages/** | ✅ shipped | `kerf-core` provides FastAPI app factory + `kerf.plugins` entry-point loader (`packages/kerf-core/src/kerf_core/app.py`, `plugin.py`, `utils/topo_sort.py`). 19 plugin packages under `packages/`: kerf-core, kerf-sdk, kerf-auth, kerf-api, kerf-chat, kerf-v1, kerf-billing, kerf-cloud, kerf-cad-core, kerf-tess, kerf-fem, kerf-cam, kerf-topo, kerf-mates, kerf-bim, kerf-electronics, kerf-imports, kerf-render, kerf-workers. Each declares `async register(app, ctx) -> PluginManifest`. Root `pyproject.toml` declares persona extras (`api-only` / `mech` / `electronics` / `bim` / `full` / `compute-only`) and a uv workspace. Cloud-gated plugins (kerf-billing, kerf-cloud) early-return dormant manifests when `ctx.cloud_enabled=False`. Test suite: 864 passing / 21 skipped across all packages. |
...
| **3D assembly mates (Tier 0 foundation)** | ✅ shipped | Coincident / concentric / parallel / perpendicular / distance / angle / tangent. Pure-Python `GeometricConstraintSolver` (gradient-descent, 100-iter) in `packages/kerf-mates/src/kerf_mates/solver.py`. LLM tools: `add_mate`, `delete_mate`, `list_mates`, `solve_assembly`. pyworker route `/run-mates`. REST endpoint `POST /api/projects/{pid}/files/{fid}/solve-mates`. `.assembly` JSON `mates: [{type, a:{component_id, feature, feature_id}, b:{…}, value?}]`. Frontend: `MatesPanel.jsx` collapsible card inside AssemblyEditor (type selector, A/B ref rows with manual text + BREP face/edge viewport picker, Add/Delete, live solve result chip). Pick flow: "Pick from viewport" → Editor enters face-pick mode → amber banner "Click a face or edge on ref A/B…" with Esc cancel → `mateRefFromPick()` converts Renderer hit → ref auto-populated. `mateRefFromPick` exported from `src/lib/assembly.js`; strips `/origPartId` suffix for multi-object components. Vitest cases in `assembly.test.js` + `matesPanel.test.js`. |
| **Scripting: `.script.py` via `kerf-sdk`** | ✅ shipped | `/v1/rpc` JSON-RPC dispatcher in `packages/kerf-v1/src/kerf_v1/routes.py` over the existing LLM tool registry (method→tool map for files / equations / configurations / revisions / docs); `script` file kind + `extension` column (migrations `026_kind_script.sql` + `029_script_extension.sql`); `api_tokens` table (migration `025_api_tokens.sql`) with `scopes jsonb` + `last_used_at` + soft-revoke; create/list/revoke endpoints in `packages/kerf-auth/src/kerf_auth/routes.py` mounted at `/api/api-tokens`; `WorkspaceSettings.jsx` API Tokens card (issue/list/revoke, "shown once on creation" warning + copy); `ScriptEditor.jsx` recognizes `.script.py` and shows `pip install kerf-sdk` install hint; `packages/kerf-chat/llm_docs/script.md` documents the surface. **kerf-sdk/ in-monorepo**: `packages/kerf-sdk/`. `kerf-sdk/pyproject.toml` (hatchling, `httpx>=0.27`); `src/kerf/{__init__,auth,client,tools}.py` — `connect()` / `from_env()` / `Kerf`, JSON-RPC 2.0 envelope, namespaced wrappers `kerf.files.*` / `kerf.equations.*` / `kerf.configurations.*` / `kerf.revisions.*` / `kerf.docs.*`; `tests/test_client.py` 7 respx-mocked tests; `.github/workflows/publish.yml` PyPI trusted-publish on `sdk-v*` tags. Local install: `pip install -e kerf-sdk/`. Decision: monorepo eliminates contract drift. |
| ↳ **TypeScript SDK (npm: kerf-sdk)** | ✅ shipped | `packages/kerf-sdk-ts/`. Native `fetch`-based JSON-RPC 2.0 client; ESM-only, Node 18+; `connect()` / `fromEnv()` / `Kerf` / `KerfError`; namespaced wrappers for all 15 `METHOD_TO_TOOL` methods (`k.files.*`, `k.equations.*`, `k.configurations.*`, `k.revisions.*`, `k.docs.*`); `AsyncDisposable` (`await using`) support; 8 vitest tests (envelope, auth header, error mapping, namespace wrapper, `fromEnv`, missing token, HTTP error, `AsyncDisposable`); `.github/workflows/publish.yml` npm-publish on `sdk-ts-v*` tags + GitHub Release. Same wire format as Python SDK — both target `POST /v1/rpc`. |
| ↳ **Rust SDK (crates.io: kerf-sdk)** | ✅ shipped | `packages/kerf-sdk-rs/`. `reqwest`-based JSON-RPC 2.0 client, async via `tokio`. Namespaced struct methods mirroring Python + TS SDKs. `from_env()` reads `KERF_API_TOKEN` + `KERF_API_URL`. `KerfError` enum with explicit variants per RPC error code (`Unauthorized`, `NotFound`, `RateLimited`, `RpcError`, `Http`, `InvalidResponse`, `MissingEnv`). `Kerf::new(url, token)` + `Kerf::from_env()`. Public fields: `k.files`, `k.equations`, `k.configurations`, `k.revisions`, `k.docs` — each a cloneable struct with async methods. `serde`-typed response structs (`FileInfo`, `FileContent`, `EquationsMap`, `RevisionInfo`, `DocHit`, etc.) with `#[serde(flatten)]` for forward-compatibility. `wiremock`-based integration tests (15 cases: auth header, all namespaces, error-code mapping). `.github/workflows/publish-sdk-rs.yml` crates.io publish on `sdk-rs-v*` tags. Same wire format as Python + TS — all three target `POST /v1/rpc`. Auto-doc via `cargo doc`. No blocking variant — Tokio 1.x is universal for Rust async; a `blocking::` re-export is deferred to a later minor. |
| **E2E Playwright tests** | ✅ shipped | `tests/e2e/` with Playwright. 3 golden-path specs (projects / sketch / jscad) + 5 scaffolded stubs (signup / feature / workshop / library / drawing) for future PRs. `playwright.config.ts` boots kerf-server on :8081 + Vite on :5174. CI workflow (`.github/workflows/e2e.yml`) runs against a fresh Postgres service container with `KERF_LOCAL_MODE=true` so no sign-in step is needed — the app auto-bootstraps a singleton user via `/auth/bootstrap-local`. Root `package.json` gains `test:e2e` + `test:e2e:ui` scripts. |
| ↳ **Go SDK (`github.com/kerf-sh/kerf-sdk-go`)** | ✅ shipped | `packages/kerf-sdk-go/`. Standard-library `net/http` JSON-RPC 2.0 client (zero third-party deps). `kerf.FromEnv()` + `kerf.New(apiURL, apiToken)`. Namespaced sub-packages: `kerf/files`, `kerf/equations`, `kerf/configurations`, `kerf/revisions`, `kerf/docs` — each defines its own `Caller` interface to avoid import cycles. `*kerf.Error` with `Code`/`Message`/`Data`; `errors.Is` matching on `Code`; sentinel vars `ErrUnauthorized` / `ErrNotFound` / `ErrRateLimited` / `ErrMissingEnv`. `context.Context` on every method. `client_test.go` (8 `httptest` cases: auth header, 3 RPC error mappings, network failure, `FromEnv` missing/present); per-namespace happy-path tests. `.github/workflows/publish-sdk-go.yml` tag-push on `sdk-go-v*`. Same wire format as Python + TS + Rust SDKs. |
| ↳ **Lua SDK (LuaRocks: kerf-sdk)** | ✅ shipped | `packages/kerf-sdk-lua/`. Pure-Lua JSON-RPC 2.0 client over `luasocket` + `luasec` (no C extensions beyond those two). `kerf.from_env()` + `kerf.connect{api_url, api_token}`. Colon-syntax namespaced tables: `k.files:list(project_id)`, `k.equations:read(...)`, etc. All methods return `(result, err)` — `result == nil` on error, `err` is a `KerfError` table `{code, message, data}`. Error codes: `UNAUTHORIZED=-32001`, `NOT_FOUND=-32004`, `RATE_LIMITED=-32429`, `RPC_ERROR=-32603`, `MISSING_ENV=-32000`. Modules: `kerf/init.lua`, `kerf/client.lua`, `kerf/error.lua`, `kerf/files.lua`, `kerf/equations.lua`, `kerf/configurations.lua`, `kerf/revisions.lua`, `kerf/docs.lua`. `kerf-sdk-0.1.0-1.rockspec` lists deps + build map. `spec/client_spec.lua` — 12 vanilla-Lua test cases (no busted required): auth header, RPC envelope, error-code mapping (`-32001/-32004/-32429`), HTTP 500, `from_env` missing/present, namespace wrappers for `files`/`equations`/`docs`, trailing-slash strip, `error.__tostring`. `examples/quickstart.lua`. `.github/workflows/publish-sdk-lua.yml` — `leafo/gh-actions-lua@v10` + `leafo/gh-actions-luarocks@v4` → `luarocks upload` on `sdk-lua-v*` tags. Target Lua 5.1+ / LuaJIT (widest embedded host compatibility — Blender, Rhino, LÖVE, Redis scripting). Same `/v1/rpc` wire format as Python + TS + Rust + Go SDKs. |
...
| **FEM: mechanical analysis** | ✅ shipped | **FEniCSx primary** (LGPL3, Python-native, active UK/US consortium dev, UFL differentiable, multiphysics-natural, GPU on roadmap); CalculiX second-solver behind same `pyworker` `/run-fem` route for FreeCAD-bit-exact compat + frictional contact when demanded. Gmsh for meshing in both. `.fem` file kind wired: `POST/GET /api/.../fem` + `/fem/status`, `fem_run` / `fem_job_status` LLM tools, `FEMView.jsx` wired in `Editor.jsx`, gmsh/dolfinx/ccx gated with `ENGINE_PENDING_WARNING` sentinels. **dolfinx UFL forms wired**: real small-strain linear elasticity (σ = λ tr(ε) I + 2μ ε, Dirichlet + Neumann BCs, von Mises post-processing, FoS). Bonded contact via shared-node conformal Gmsh mesh (`occ.fragment()`). Returns `node_displacements` per-node vector + `stresses` per-cell DG0 array. **Deformed-shape 3D overlay** in `FEMView.jsx`: Three.js morphed by `displacement * scale_factor`; UI scale slider 1–200×; colour by displacement magnitude or von-Mises stress; colour bar legend; toggle show/hide. **BufferGeometry passthrough**: `Editor.jsx` extracts display mesh from current `w.parts` via `extractDisplayGeometryFromParts` and passes it to `FEMView` → `FEMDeformedShape` so a lit + wireframe morphed surface is rendered instead of the point-cloud proxy when a source part is loaded. **Modal analysis SLEPc wired**: GHEP K x = λ M x via `SLEPc.EPS` (Krylov-Schur + SINVERT shift-invert); frequencies Hz = sqrt(λ)/(2π); mode shapes returned; `_SLEPC_AVAILABLE` gate. **CalculiX modal wired**: `*FREQUENCY,NMODES=N` step; `.dat` eigenvalue parser (ω² → Hz); `.frd` mode-shape parser; same result schema (`frequencies` + `mode_shapes`) as FEniCSx path; `_CALCULIX_AVAILABLE` gate returns `ENGINE_PENDING_WARNING` when `ccx` absent. **Multi-body multi-material BCs**: `material_props` accepts `[{body_tag, material_props}]` list; Lamé + density assembled as piecewise-constant DG0 fields per cell-tag region; backwards-compat with single-dict form. Pytests: cantilever beam (Euler-Bernoulli, 5% tol) + FEniCSx modal (first bending freq analytical, 5% tol, `importorskip` SLEPc) + CalculiX modal (`test_fem_calculix_modal.py`, `shutil.which` skip). Vitest: `extractDisplayGeometryFromParts` (JSCAD Geom3 + BufferGeometry cases) + displacement-scaling math. |
| **Tolerance stack-up** | ✅ shipped | `.tolerance` JSON file kind; `tolerance_stack` + `tolerance_monte_carlo` LLM tools; `worst_case` / `rss` / `monte_carlo` pure helpers in `packages/kerf-mates/src/kerf_mates/tolerance.py`; `POST /api/projects/{pid}/files/{fid}/tolerance/run` API route (worst_case / rss / monte_carlo methods); frontend `src/lib/tolerance.js` (pure functions for in-browser compute); `ToleranceView.jsx` — dimension-chain table, WC+RSS summary cards, inline SVG histogram for Monte-Carlo output, Run button; wired in `Editor.jsx` for `.tolerance` file kind. **Automatic chain-walk shipped**: `tolerance_auto_chain` LLM tool (registered via `packages/kerf-mates/src/kerf_mates/plugin.py`) walks the assembly mate graph by BFS between two feature references and auto-builds the dimension chain. Pure helper `build_chain_from_assembly` in `packages/kerf-mates/src/kerf_mates/chain_walk.py`. Per-mate `tolerance: {plus, minus}` slot added to `.assembly` schema (backward-compatible). `packages/kerf-chat/llm_docs/tolerance.md` documents full schema + all methods + chain-walk. 12 pytest-style tests in `packages/kerf-mates/tests/test_tolerance_chain_walk.py`. |
...
| **NURBS surfacing (Phase 4)** | 🚧 in flight | sweep1/sweep2/networkSrf/blendSrf surface creation tools + display shipped. Python NurbsSurface layer (`packages/kerf-cad-core/src/kerf_cad_core/geom/`). LLM tools `feature_sweep1/2/network_srf/blend_srf` + `surface_continuity` query/enforce tool (C0/C1/C2 for sweeps/network; G0/G1/G2 for blend). `.surf` file kind documented. NURBS tessellated for display by OCCT worker. **v1 booleans activated** (user-demand signal received): small-scope path — sweep-to-solid capping helper (`feature_to_solid`) + `feature_boolean` op wrapping `BRepAlgoAPI_Cut_3` / `Fuse_3` / `Common_3` on solids. ~6 sonnet-agent-days (T1–T7). Multi-year framing remains accurate for full NURBS-NURBS robustness corpus + trim-by-curve + matchSrf + G3 continuity (all still deferred). Scoping pass: [docs/plans/nurbs-booleans-scoping.md](./docs/plans/nurbs-booleans-scoping.md). Implementation plan: [docs/plans/nurbs-booleans-v1.md](./docs/plans/nurbs-booleans-v1.md). |
| ↳ **T1: Binding probe + `surfaceToSolid` helper** | ✅ shipped | One-shot probe added to `loadOcct()` in `src/lib/occtWorker.js`; logs `[occt-bindings] <Class>: OK|MISSING` for all 3 gating classes at boot. `getNurbsBooleanBindings(oc)` exported for programmatic branching by T4. `surfaceToSolid(oc, shape, tracker, opts)` + `SurfaceToSolidUnsupportedError` added to `src/lib/occtBridge.js`. 27 unit tests in `src/__tests__/surfaceToSolid.test.js` (all green, no WASM required). **Binding-presence verdict (static analysis only — real build not run):** `BRepBuilderAPI_Sewing` → **can't determine without running worker** (not referenced anywhere in `src/`); `BRepBuilderAPI_MakeSolid_1` → **can't determine without running worker** (not referenced anywhere in `src/`); `BRepAlgoAPI_Common_3` → **can't determine without running worker** (not referenced, but follows identical constructor shape as `Cut_3`/`Fuse_3` which are bound). Fallback paths implemented: (1) `Sewing` missing → `SurfaceToSolidUnsupportedError` thrown (T2+ blocked); (2) `MakeSolid_1` missing or `IsDone()` false → `BRep_Builder.MakeSolid + Add` hand-roll; (3) `Common_3` missing → T4 uses `A − (A − B)` identity (documented in design doc). **Next step for T2/T4 agents:** run the worker in a browser context once and look for the `[occt-bindings]` lines in the console to determine the primary-vs-fallback path. If `Sewing` logs `MISSING`, pause and escalate for WASM rebuild. |
| ↳ **T2: `opToSolid` worker handler + dispatch** | ✅ shipped | `opToSolid` added to `src/lib/occtWorker.js`; `case 'to_solid'` wired into **both** `evaluateTree` and `evaluateToFinalShape`. Wraps T1's `surfaceToSolid` helper; respects the boot-time `NURBS_BOOLEAN_BINDINGS` probe (fast-fail when `BRepBuilderAPI_MakeSolid_1` reports missing); `SurfaceToSolidUnsupportedError` routes through the dispatch's outer catch to the worker error envelope. 25 unit tests in `src/__tests__/toSolidWorker.test.js` (no WASM required — `oc` surface is mocked). |
| ↳ **T3: `feature_to_solid` Python LLM tool** | 📋 next | Add `feature_to_solid_spec` + `run_feature_to_solid` to `packages/kerf-cad-core/src/kerf_cad_core/surfacing.py` (template: `run_feature_blend_srf`). Register in `_TOOL_MODULES`. New doc page `packages/kerf-chat/llm_docs/feature_to_solid.md`. Deps: none. |
| ↳ **T4: `opBoolean` worker handler** | 📋 next | Add `opBoolean` to `src/lib/occtWorker.js`; switch on `node.kind ∈ {cut, fuse, common}` → dispatch to `BRepAlgoAPI_Cut_3` / `Fuse_3` / `Common_3`. Wire `case 'boolean'` into both evaluators. If `Common_3` missing, use the `A − (A − B)` identity fallback. Surface-operand inputs error with hint pointing at `feature_to_solid`. Deps: T2. |
| ↳ **T5: `feature_boolean` Python LLM tool** | 📋 next | Add `feature_boolean_spec` + `run_feature_boolean` to `surfacing.py`. Register in plugin. New doc page `feature_boolean.md`. Deps: none functionally; ordered after T4 for design-doc cohesion. |
| ↳ **T6: FeatureView inspector entries** | 📋 next | Add `to_solid` + `boolean` entries to `FEATURE_KINDS` in `src/components/FeatureView.jsx`; add both ops to the `Modify` category. `to_solid` inspector: `target_id` (feature_picker) + `tolerance` (number). `boolean` inspector: `target_a_id` / `target_b_id` (feature_picker each) + `kind` (select: cut/fuse/common). Deps: T2, T4. |
| ↳ **T7: Pytest + vitest + integration test** | 📋 next | `packages/kerf-cad-core/tests/test_feature_to_solid.py` + `test_feature_boolean.py` for schema/error coverage. `src/__tests__/featureBoolean.test.js` for dispatch + inspector. `src/__tests__/booleanIntegration.test.js` (WASM-gated like `occtRunner.test.js`) for end-to-end: sweep+pad cut, blend_srf cap+fuse, surface-operand error path. Deps: T3, T5, T6. |
| **NURBS surfacing Phase 4 — full Capability 1 (Robust NURBS-NURBS booleans)** | ✅ shipped | Surface-direct boolean path that accepts Face/Shell operands without a solid round-trip. T1–T6 shipped (see sub-rows). T7–T10 remain as planned escalation paths; C1-T10 (WASM rebuild) only triggers if `BRepAlgoAPI_Cut_3` rejects non-solid operands at runtime. Design doc: [docs/plans/nurbs-phase-4-full.md](./docs/plans/nurbs-phase-4-full.md) § Capability 1. |
| ↳ **C1-T1: Binding probe extension** | ✅ shipped | Extended `NURBS_BOOLEAN_BINDINGS` probe in `src/lib/occtWorker.js` with 13 new entries covering all four Phase 4 capabilities. Exports `getNurbsPhase4Bindings(oc)` (structured per capability group). Logs `[occt-phase4] <Cap> — <Class>: OK\|MISSING` + `gate: GO\|PARTIAL/BLOCKED` per group at boot. C1 gating classes: `BOPAlgo_Builder`, `BRepAlgoAPI_Section`, `ShapeFix_Shape`, `ShapeFix_Solid`, `ShapeUpgrade_UnifySameDomain`. C2–C4 classes also probed for future capability owners. **Binding-presence verdict (static analysis only — real WASM build not run):** all five C1 classes are unverified; presence determined at runtime. If any core class (e.g. `BRepAlgoAPI_Section`, `BOPAlgo_Builder`) logs MISSING, opSurfaceBoolean's optional features degrade gracefully; if `Cut_3`/`Fuse_3` reject non-solid operands (C1-T10 risk), the worker surfaces a clear escalation error. 21 probe tests in `src/__tests__/surfaceBooleanProbe.test.js`. |
| ↳ **C1-T2: `opSurfaceBoolean` worker handler** | ✅ shipped | New `opSurfaceBoolean` in `src/lib/occtWorker.js`. Accepts any `TopoDS_Shape` topology (no solid enforcement). Probe-gated `ShapeFix_Shape` pre-pass + `SetFuzzyValue` + `ShapeUpgrade_UnifySameDomain` cleanup. `Common_3` absent → `A − (A − B)` identity fallback. `IsDone()` false → clear C1-T10 escalation error. `case 'surface_boolean'` wired into both `evaluateTree` and `evaluateToFinalShape`. Existing `opBoolean`/`opToSolid` handlers untouched. 28 dispatch + error + probe-gate tests in `src/__tests__/surfaceBoolean.test.js` (no WASM). |
| ↳ **C1-T3: `feature_surface_boolean` Python tool** | ✅ shipped | `feature_surface_boolean_spec` + `run_feature_surface_boolean` added to `packages/kerf-cad-core/src/kerf_cad_core/surfacing.py`. Fields: `file_id`, `target_a_id`, `target_b_id`, `kind` (cut/fuse/common), `fuzziness` (optional, default absent = 1e-4 in worker), `coarse_mode` (optional bool). Node op = `"surface_boolean"`. Auto-id prefix `"surface_boolean-"`. Fuzziness zero/negative rejected with BAD_ARGS. Registered via `@register` decorator. LLM doc page: `packages/kerf-chat/llm_docs/feature_surface_boolean.md`. 35 pytest cases in `packages/kerf-cad-core/tests/test_feature_surface_boolean.py`. |
| ↳ **C1-T4: FeatureView inspector entry** | ✅ shipped | Added `surface_boolean` to `FEATURE_KINDS` in `src/components/FeatureView.jsx` — placed adjacent to `boolean` (solid), conceptual siblings. Inspector fields: `target_a_id` (feature_picker), `target_b_id` (feature_picker), `kind` (select: cut/fuse/common), `tolerance` (number, default 1e-3), `fuzzy_value` (number, optional). Caption block under the fields explains surface-direct vs solid-boolean semantics. Added to `FEATURE_CATEGORIES.surface`. Inspector `caption` rendering support added to `FeatureInspector`. |
| ↳ **C1-T5: Integration tests** | ✅ shipped | New `src/__tests__/surfaceBooleanIntegration.test.js`: 6 source-level checks (run unconditionally) + 6 WASM-gated scenarios (skipped in Node, run in CI). Scenarios: BSpline patch cut, blend_srf+sweep1 fuse, solid-input error path, IsDone=false C1-T10 escalation, Common_3 fallback identity, T6 performance timing (50×50 pole-grid). Extended `surfaceBoolean.test.js` with +10 mock cases for T4 inspector field plumbing (fuzzy_value/tolerance/fuzziness priority, coarse_mode gates, target bodyMap validation). Extended `test_feature_surface_boolean.py` with +11 pytest cases (coarse_mode schema + storage, fuzzy_value threading, multi-node auto-id). 47 vitest + 35 pytest all pass; 6 WASM scenarios skipped as expected in Node. |
| ↳ **C1-T6: Worker performance pass** | ✅ shipped | Static analysis confirmed `opSurfaceBoolean` was already minimal (one ShapeFix pre-pass, one Build, one optional Unify — no redundant calls). Added opt-in `coarse_mode: true` flag that skips `ShapeFix_Shape` pre-pass and `ShapeUpgrade_UnifySameDomain` cleanup (~30-50% faster on dense NURBS, may produce non-watertight fragments). Inspector field priority for fuzziness: `fuzzy_value` > `fuzziness` > `tolerance` > 1e-4. `coarse_mode` added to Python tool spec and handler. **WASM timing unavailable in Node env** — expected on typical jewelry NURBS (50×50 pole-grid): `cut: ~500ms · fuse: ~500ms · common: ~800ms`; `coarse_mode` reduces each by ~30-50%. Sub-2s goal expected met. `coarse_mode` needed? Not required — added as additive performance opt-in, not a correctness fix. |
| ↳ **C1-T7 through C1-T10** | 🔮 planned | C1-T7: vitest worker dispatch validation (source checks now covered in T5). C1-T8: WASM CI harness with real geometry (placeholder tests in T5). C1-T9: `SetFuzzyValue` absent fallback (already handled gracefully). C1-T10: custom WASM rebuild escalation — only triggers if `BRepAlgoAPI_Cut_3` rejects Face/Shell operands at runtime (plan Q1 open question). **Plan Q1 resolution:** `BRepAlgoAPI_Cut_3` accepting Face/Shell cannot be answered without running WASM — runtime IsDone=false triggers the clear C1-T10 escalation message. |
| **NURBS surfacing Phase 4 — Capability 2 (Trim-by-curve)** | 🚧 in flight | T1-T3 shipped (see sub-rows). T4 (inspector), T5 (integration tests), T6 (perf) are v0.3. Design doc: [docs/plans/nurbs-phase-4-full.md](./docs/plans/nurbs-phase-4-full.md) § Capability 2. Persistent-naming caveat: `trim_by_curve` invalidates positional `face-N` IDs — downstream ops referencing the trimmed face break on re-eval until persistent-face-naming ships. |
| ↳ **C2-T1: Binding probe extension** | ✅ shipped | Extended `NURBS_PHASE4_C2_BINDINGS` array in `src/lib/occtWorker.js` with 7 new entries covering the per-point projection fallback path: `GeomAPI_ProjectPointOnSurf`, `ShapeAnalysis_Surface`, `BRepBuilderAPI_MakeEdge`, `BRepBuilderAPI_MakeWire`, `BRepBuilderAPI_MakeFace`, `ShapeFix_Wire` (in addition to existing `BRepFeat_SplitShape`, `BRepProj_Projection`, `BRepBuilderAPI_MakeFace_18`). New export `getNurbsPhase4C2Bindings(oc)` returns C2-only binding map. New `_logNurbsPhase4C2Bindings(oc)` standalone logger. Boot-time logging via existing `_logNurbsPhase4Bindings` groups (already logs C2 group). **Binding-presence verdict (static analysis only — real WASM build not run):** `BRepFeat_SplitShape` is the highest-risk class (niche `BRepFeat` module; may be absent per plan Q1). If MISSING, `opTrimByCurve` throws `TrimByCurveUnsupportedError` with a C2-T12 escalation hint. 24 probe tests in `src/__tests__/trimByCurveProbe.test.js`. |
| ↳ **C2-T2: `opTrimByCurve` worker handler** | ✅ shipped | New `opTrimByCurve` in `src/lib/occtWorker.js`. Resolves target face by `target_face_name` (positional `face-N` id) via `TopExp_Explorer`. Resolves cutter curve from `sketches` or `bodyMap`. Calls `projectCurveOntoSurface` (new helper in `occtBridge.js`): primary `BRepProj_Projection`, fallback 32-point `GeomAPI_ProjectPointOnSurf` sample-and-project loop → `BRepBuilderAPI_MakeEdge` + `MakeWire`. Calls `splitFaceAlongCurve` (new helper in `occtBridge.js`): primary `BRepFeat_SplitShape.Left()/Right()`, throws `TrimByCurveUnsupportedError` with C2-T12 escalation if absent. `keep_side: "positive"|"negative"` selects which half to return. New exports: `TrimByCurveUnsupportedError`, `projectCurveOntoSurface`, `splitFaceAlongCurve`. `case 'trim_by_curve'` wired into both `evaluateTree` and `evaluateToFinalShape`; does NOT cleanup `current` before dispatch (opTrimByCurve reads target face from it). Existing `opSurfaceBoolean`/`opSection` handlers untouched. 36 dispatch + mock + bridge-wiring tests in `src/__tests__/trimByCurve.test.js` (no WASM). |
| ↳ **C2-T3: `feature_trim_by_curve` Python tool** | ✅ shipped | `feature_trim_by_curve_spec` + `run_feature_trim_by_curve` added to `packages/kerf-cad-core/src/kerf_cad_core/surfacing.py`. Fields: `file_id`, `target_feature_ref`, `target_face_name`, `trim_curve_ref` (required); `keep_side` (`"positive"|"negative"`, default `"positive"`), `tolerance` (optional positive float), `options.id`. Node op = `"trim_by_curve"`. Auto-id prefix `"trim_by_curve-"`. `keep_side` invalid value rejected with BAD_ARGS. `tolerance` zero/negative rejected. Registered via `@register` decorator. LLM doc page: `packages/kerf-chat/llm_docs/feature_trim_by_curve.md` — schema, algorithm, jewelry use case example, binding-probe gate table, persistent-naming caveat, error code table. 30 pytest cases in `packages/kerf-cad-core/tests/test_feature_trim_by_curve.py`. |
| ↳ **C2-T4 through C2-T12** | 🔮 planned | C2-T4: inspector entry (`FeatureView.jsx`). C2-T5: integration tests (3 WASM-gated scenarios). C2-T6: performance pass. C2-T7 (LLM doc) already shipped as part of T3. C2-T8/T9/T10: inspector face-pair picker (shared with C3-T7). C2-T11: WASM integration (sketch-cuts-hole-in-blend, edge-derived-cutter, degenerate-cutter error). C2-T12: fallback path if `BRepFeat_SplitShape` absent (Section+prism approach — ~3 sonnet-days). |
| **NURBS Phase 4 — Capability 4 (G3 curvature combs visualization)** | ✅ shipped | Three.js overlay rendering principal-curvature combs on NURBS surfaces. Users EYEBALL G3 continuity at face junctions (blend tangent verification — jewelry / automotive ID). Per Phase 4 plan: algorithmic G3 enforcement structurally impossible in stock OCCT (no GeomAbs_G3 value); viz-only is the recommended path. opSurfaceCurvatureCombs worker + feature_surface_curvature_combs Python tool + CurvatureCombOverlay component + FeatureView inspector. `sampleSurfaceCurvature(oc, face, uvDensity, tracker)` in `occtBridge.js` samples a UV grid via `GeomLProp_SLProps_2` (order=2); returns per-point {k1, k2, mean=(k1+k2)/2, gaussian=k1×k2, maxAbs=max(|k1|,|k2|), normal, principalDir} + stats envelope. Colormap: blue (concave, mean<0) → white (flat) → red (convex, mean>0), auto-scaled to sample range. Toggle + UV-density slider + scale-factor input in `CurvatureCombPanel`. Binding probe: `GeomLProp_SLProps` in `NURBS_PHASE4_C4_BINDINGS`; constructor form already confirmed bound by 5-axis CAM and surface_continuity code. **Honest gap**: full algorithmic G3 requires either (a) custom WASM rebuild adding a G3-aware constraint solver in C++, or (b) iterative pole-adjustment Python approximation — neither is stock OCCT. 58 vitest (dispatch + colormap + buildCombGeometry + probe) + 24 pytest schema tests. |
| **5-axis CAM real solver** | ✅ shipped | Constant-tilt surface finishing + 3+2 indexed; full pipeline from STEP/BRep → drive-face extraction → CL generation → G-code emission with A/B rotary axes. Both LinuxCNC and Fanuc posts. Tool DB (7 types) wired into both strategies. T1–T8 complete (8 tasks, ~17 sonnet-agent-days). Deferred: swarf, Sandvik morphing, additional kinematic posts (head-head, table-table), gouge/collision detection (CAMotics follow-on). Full design: [docs/plans/5-axis-cam.md](./docs/plans/5-axis-cam.md). |
| ↳ **T1: OCL native 5-axis primitive audit** | ✅ shipped | Static binary audit of `opencamlib 2023.1.11` PyPI wheel (`ocl.so` polymorphic-type-registry + string extraction). Confirmed: zero 5-axis primitives; all 41 exposed classes are 3-axis or geometry-utility. `CLPoint` / `CCPoint` carry position only — no tool-axis vector, no surface normal. Design-doc assumption ("OCL has no native 5-axis primitive — layer on pythonOCC `GeomLProp_SLProps`") verified. Probe script: `scripts/probe_ocl_5axis.py`. Audit report: [docs/plans/5-axis-cam-ocl-audit.md](./docs/plans/5-axis-cam-ocl-audit.md). T2–T8 designs unchanged. Open question for T2: verify `GeomLProp_SLProps` constructor variant in installed pythonocc-core version. |
| ↳ **T2: GeomLProp_SLProps drive-face normal extraction** | ✅ shipped | New `packages/kerf-cam/src/kerf_cam/five_axis/drive_face.py`: `extract_drive_face(brep_path, face_id)` (STEP/BRep load + face selection by TopExp index), `surface_normal_at(face, u, v)` (calls SLProps, returns (Point3, Vec3) or None for degenerate UV), `uv_iso_curves(face, step_over_mm)` (UV grid sampling). Constructor probe: Form A `GeomLProp_SLProps(surf, u, v, 1, 1e-6)` confirmed working (same form already used in `routes.py:246`). Tests: `test_drive_face.py` — box top face normal is ±Z, cylinder lateral normal is radial, iso-curve counts/bounds, sphere-pole IsNormalDefined graceful None. All 9 tests skip cleanly without pythonOCC. |
| ↳ **T3: Constant-tilt finishing CL generation** | ✅ shipped | New `packages/kerf-cam/src/kerf_cam/five_axis/constant_tilt.py`: `run_constant_tilt(spec) -> CAMResult`. UV iso-curve walk + per-point SLProps normal + Rodrigues tilt-about-tangent + ball-end tip math (`ball_centre = p + r*n`, `tip = ball_centre - r*axis`). Optional `lead_deg` lead/lag tilt about the n×tangent cross axis. Returns `{cl_points: [{x,y,z,i,j,k},...], warnings, skipped_uv}`. Rejects `tilt_deg` outside [0,30]. Tests: `test_constant_tilt.py` — tilt=0 flat face: all axes ±Z; tilt=15 flat face: all axes 15° off normal; tip.z == surface_z for tilt=0 ball; skipped_uv=0 on non-degenerate plane. 7 tests skip cleanly without pythonOCC. |
| ↳ **T4: 3+2 indexed via STL rotation to drive-face Z** | ✅ shipped | New `packages/kerf-cam/src/kerf_cam/five_axis/indexed_3_2.py`: `run_3_2_indexed(spec) -> CAMResult`. Pure-Python `rotation_from_to(src, dst)` (Rodrigues, handles antiparallel via min-dot perp-axis selection), `apply_rotation_matrix`, `_rotate_stl_triangles`. Loads ASCII STL, rotates all triangles by R, builds ocl.STLSurf, dispatches to `_run_parallel_3d` / `_run_waterline` / `_run_ocl_op`. Returns `{cl_points, rotation_matrix, rotated_normal, warnings}` with mandatory R7 no-collision warning. Tests: `test_3_2_indexed.py` — 8 pure-Python rotation tests (always run, no deps): identity, 45°, -Z→+Z (180° antiparallel), arbitrary normals, orthogonality, STL face-normal→Z, zero-normal error, key T4 assertion (45°-tilted-plane rotated_normal=(0,0,1) within 1e-9). 2 opencamlib-gated integration tests. |
| ↳ **T5: G-code emission for constant-tilt CL points** | ✅ shipped | New `packages/kerf-cam/src/kerf_cam/five_axis/gcode_constant_tilt.py`: `emit_gcode_constant_tilt(cl_points, post, opts) -> str` + `PostOpts` dataclass. Converts per-point `(i,j,k)` tool-axis unit vector to `(A,B)` rotary-axis angles using head_table conventions (`B=atan2(sqrt(i²+j²),k)`, `A=atan2(j,i)`). Continuous A-angle unwrap: folds consecutive raw-A values into ±180° window so no slewing discontinuity. Near-singularity (B≈0, k≥cos(1°)) holds previous A + emits warning comment in G-code. Per-point feed override via optional `"feed"` key on CLPoint dicts. Two post-processors: **LinuxCNC** (`posts/linuxcnc_5x.py`) — G94 feed-per-minute, `%` tape markers, G43.4 on use_tcp, M8/M7 coolant, mandatory no-collision-check warning comment; **Fanuc** (`posts/fanuc_5x.py`) — N-line sequence numbers (opt-out via `no_n_numbers`), parenthetical comments, G05.1 Q1/Q0 AICC + G43.4 when use_tcp. `machine_kinematic="head_table"` is default and only supported value (`table_table`/`head_head` raise `NotImplementedError`). G93 inverse-time feed **not shipped** (G94 only — LinuxCNC ≥2.8 trajectory planner enforces joint limits automatically; G93 math is kinematic-family-specific and deferred). New route `POST /run-5axis` in `routes.py` accepts precomputed CL points + post choice + PostOpts directly. `CAMOperation` model extended with `drive_face_id`, `tilt_deg`, `lead_deg`, `indexed_op`, `kinematic_family`, `use_tcp`, `post_processor_5x` fields. `cam_run` LLM tool operation enum extended: `5axis_finish`, `3plus2` added (`5axis` kept as alias per R5). `5axis` stub short-circuit replaced with real dispatch to `_run_5axis_finish_route`. `llm_docs/cam_5axis.md` added. Tests: `packages/kerf-cam/tests/test_5axis_gcode.py` — 28 tests (all no-dep, always run): angle math, A/B angle values in G-code (15° tilt in X/Y), continuous-unwrap no-discontinuity assertion over a 340° azimuth sweep, feed-rate default + per-point override, zero-tilt singularity (B=0, A held), singularity warning present, TCP mode G43.4 + AICC, unknown post ValueError, unsupported kinematic NotImplementedError, empty CL points both posts. `test_cam_advanced.py` stub test replaced with `test_5axis_returns_real_toolpath_response`. |
| ↳ **T6: G-code emission for 3+2 indexed CL points** | ✅ shipped | New `packages/kerf-cam/src/kerf_cam/five_axis/gcode_indexed_3_2.py`: `emit_gcode_indexed_3_2(cl_points, post, opts) -> str`. Emits A/B rotary angles ONCE in an orientation move (`G0 A<a> B<b>`) at the top; body `G1` lines carry X/Y/Z only — A/B are never repeated mid-program. Axis-aligned short-circuit: when the drive-face normal is `≈+Z` (A=B=0), the orientation move is skipped entirely and plain 3-axis G-code is emitted. End-of-program footer always returns rotaries to home (`G0 A0.000 B0.000`) for safety (prevents next 3-axis op from running at the indexed angle). Both LinuxCNC and Fanuc posts extended with `emit_indexed_3_2` functions sharing existing tool-comment + header/footer helpers. `POST /run-5axis` extended: new `mode` field (`"constant_tilt"` | `"3plus2"`); mode dispatches to `emit_gcode_indexed_3_2` or `emit_gcode_constant_tilt` respectively. `llm_docs/cam_5axis.md` updated: new "3+2 indexed mode" section with sample input/output, when-to-use comparison table, `target_face_normal` input shape, axis-aligned short-circuit docs, and home-rotation rationale. Tests: `packages/kerf-cam/tests/test_5axis_indexed_gcode.py` — 32 tests (all no-dep, always run): one-rotation-move assertion (LinuxCNC + Fanuc), body G1 lines carry X/Y/Z only — no A/B, rotation angles A=30°/B=45° correct, footer Z50 retract + A0B0 home + M30, axis-aligned no-orientation-move (LinuxCNC + Fanuc), axis-aligned plain-3-axis body, feed-rate default + per-point override, tool-comment (T7 integration), TCP G43.4 + AICC, unknown post ValueError, unsupported kinematic NotImplementedError, empty CL points both posts, orientation-math helpers. |
| ↳ **T7: Tool DB schema + UI** | ✅ shipped | `.tool` file kind (JSON schema: `id`, `name`, `type`, `diameter_mm` + type-specific fields). 7 tool types: `ball_end`, `flat_end`, `bull_end`, `chamfer`, `drill`, `face_mill`, `engraver`. Migration `054_kind_tool.sql` extends `files.kind` check constraint. Python loader `packages/kerf-cam/src/kerf_cam/tool_db.py`: `parse_tool(data)` + `validate_tool(data)` (required fields per type + geometry sanity: ball_radius ≤ diameter/2, flute_length ≤ overall_length, rpm_min ≤ rpm_max, etc.) + async `load_tool(pool, project_id, tool_id)` + `list_tools(pool, project_id)`. `Tool.to_comment()` emits the G-code comment line. `PostOpts` extended with optional `tool` field and `apply_tool_defaults()` — feeds/rpm fall back to tool values when not explicitly overridden. Both posts emit `; tool: T1 — name, ø6.35 mm, ball r=3.175 mm, 2-flute, carbide` comment after the header. `constant_tilt.py` + `indexed_3_2.py` accept `tool_id` key: resolve ball_radius / tool_diameter from the DB when `pool` is passed, preserving backwards compat. LLM tools in `packages/kerf-cam/src/kerf_cam/tools/tool_db.py`: `create_tool`, `update_tool`, `delete_tool`, `list_tools` — registered in `plugin.py`. LLM doc page `packages/kerf-chat/llm_docs/tool_db.md`. UI: `src/components/ToolDBPanel.jsx` — tool card list with inline SVG profile thumbnails (one per type), Add/Edit modal with type-aware required-field logic, Delete. `ToolPicker` dropdown component for CAM job inspector. `CAMView.jsx` gains a Settings/Wrench tab switcher (new sidebar tab at top-right of the header, minimal footprint — no refactor of existing tabs); tool-picker row appears in the job tab when project has tools. `FileTree.jsx`: `Wrench` icon for `tool` kind + `tool` in `KIND_ROWS` / `KIND_ORDER`. Tests: `packages/kerf-cam/tests/test_tool_db.py` — 36 pytest cases (all no-dep, always run): required fields per type, geometry sanity, round-trip, `to_comment`, `effective_ball_radius`, all 7 types parse without error. `src/__tests__/toolDB.test.js` — 20 vitest cases: form validation per type, ball_radius guard, list data model, comment format. |
| ↳ **T7 ext: Tool DB wired into 3-axis posts** | ✅ shipped | New `packages/kerf-cam/src/kerf_cam/posts_common.py`: `PostOpts3` dataclass (feed_cut, feed_plunge, feed_rapid, spindle_rpm, coolant, no_n_numbers, tool) + `apply_tool_defaults()` (feeds/RPM back-filled from attached Tool when fields are at sentinel defaults) + `tool_comment_line(tool, dialect)` (semicolon for LinuxCNC/GRBL; parenthetical upper-case for Fanuc/Mach3). Four new 3-axis post files in `packages/kerf-cam/src/kerf_cam/posts/`: `linuxcnc_3x.py` (% tape markers, ; comments, G43 TLO, M6), `grbl_3x.py` (no bare M6 — comment `(M6 T<n>)` only; GRBL has no tool changer), `mach3_3x.py` (parenthetical comments, `T<n> M6` tool call, no tape markers), `fanuc_3x.py` (N-number sequence, parenthetical comments, `no_n_numbers` opt-out). All four: tool-comment line after header when Tool attached; `M6 T<opts.tool_number>` uses numeric `tool_number` (tool.id used only for the comment); feed/plunge/RPM from tool defaults with caller-override precedence; per-CL-point feed override via `"feed"` / `"feed_rate"` keys; separate rapid + plunge + cut feeds. `cam_run` LLM tool extended: new optional `tool_id` field (resolves via `tool_db.load_tool` from project pool before enqueuing the job; returns `TOOL_NOT_FOUND` if absent) + new `post_processor` field (`"linuxcnc"` default \| `"grbl"` \| `"mach3"` \| `"fanuc"`). `llm_docs/cam.md` updated: Tool DB integration section with `cam_run` extra fields, post-processor dialect table, GRBL M6 rationale, and LinuxCNC sample G-code block. Tests: `packages/kerf-cam/tests/test_3axis_tool_db.py` — 33 pure-Python cases: LinuxCNC header/tape/tool-comment/M6/feeds/spindle/override, GRBL no-bare-M6/comment-only/tool-comment/feeds, Mach3 `T<n> M6`/paren-comment/no-tape/feeds, Fanuc N-numbers/no-N/paren-comment/M6/feeds, coolant flood+mist across all 4 posts, per-point feed override, empty CL points all posts, no-tool default tool_number. All 33 pass; existing `test_tool_db.py` (36) + `test_5axis_gcode.py` (28) + `test_5axis_indexed_gcode.py` (32) stay green. |
| ↳ **T8: End-to-end integration tests** | ✅ shipped | New `packages/kerf-cam/tests/test_5axis_e2e.py` — 8 integration tests covering the full pipeline. **Fixture** generated programmatically in `conftest.py` (no binary blob committed): 50×50×10 mm box with 10×30×5 mm slot pocket created via `BRepAlgoAPI_Cut`, written to session-scoped `tmp_path` as STEP + ASCII STL. **Test A** (OCC-gated): constant-tilt full pipeline — STEP → `extract_drive_face` → `uv_iso_curves` → `run_constant_tilt` → `emit_gcode_constant_tilt(post="linuxcnc")`. Asserts header (G90 G94 G17 G21, G54, M6, M8), tape markers, ≥2 G1 lines, A/B present, no A-jump >90° (continuous unwrap), F800 on first cut move, footer (M5, M9, M30), no-collision warning. **Test B** (pure-Python): 3+2 indexed pipeline with synthetic CL data (face normal tilted 30° toward +Y); Fanuc post. Asserts exactly ONE G0 A<a> B<b> orientation move, B=30°/A=90°, body G1 lines carry no A/B words, footer homes to G0 A0.000 B0.000. **Test C** (pure-Python): Tool DB integration — parse a 1/4" ball-end tool, wire into PostOpts, assert tool-comment line with name, M6 T1, F750 from tool.feed_rate_mm_min, S8000 from tool.spindle_rpm_min. **Test D** (pure-Python): axis-aligned short-circuit — +Z face normal → no orientation move emitted, body is plain 3-axis G1, home footer still present. **Test E** (OCC-gated): corrupt STEP bytes → `RuntimeError` with non-empty message (no crash, no segfault). **Test F** (pure-Python): Fanuc post N-line numbers, parenthetical comments, M7 mist coolant, B=20° at 20° tilt. **Test G** (pure-Python): rotation-angle math — face_normal=(0,sin30°,cos30°) → orientation move B=30.000°/A=90.000° within 0.05°. **Test H** (pure-Python): no-OCC smoke test — G-code emission and tool-DB parse work end-to-end without any optional dep. Counts: 6 passed, 2 skipped (OCC absent in this env); all 8 pass when OCC is installed. **Gap found in T1–T7**: `run_3_2_indexed` depends on `opencamlib` for the 3-axis sub-op (`_triangles_to_ocl_surface` + `_run_sub_op`); Test B side-steps this by testing the G-code emitter directly with synthetic CL data. The integration path `run_3_2_indexed → emit_gcode_indexed_3_2` can only be exercised end-to-end when both `pythonocc-core` and `opencamlib` are installed — both are `[optional]` extras in `pyproject.toml`, which is intentional. |
| **Persistent face naming (Phase 4 CAD)** | 🔮 planned | Fixes positional `face-N` IDs that shuffle when sketches upstream change — affects `push_pull`, `feature_cut_from_sketch`, `feature_boss_with_draft`, fillets/chamfers. Chosen scheme: sketch-anchored primary (`Pad-A.TopCap`, `Pad-A.Side.seg-3`) + topological-signature fallback for free-floating faces (8-char hash). Dual-write `target_face_name` alongside legacy `target_face_id`; `resolveFaceRef` checks name first, falls back to integer — no flag day. 7 tasks, ~5.5 sonnet-agent-days. Highest-risk open question: boundary-face naming on boolean ops (FreeCAD's topo-naming branch has thrashed on this for years). Full design: [docs/plans/persistent-face-naming.md](./docs/plans/persistent-face-naming.md). |
| **Rhino parity: Quad remesher** | ✅ shipped | Quad-dominant remeshing via Instant Meshes CLI (MIT, subprocess pattern — graceful degradation when binary absent). `feature_quad_remesh` op appends a `quad_remesh` node to a `.feature` file. `instant_meshes_runner.py` — subprocess wrapper + OBJ I/O + `InstantMeshesNotInstalledError` (HTTP 503 with install hint). `POST /run-quad-remesh` pyworker route mounted by `kerf-cad-core` plugin. `.quadmesh` file kind (migration `058_kind_quadmesh.sql`). `QuadMeshView.jsx` — Three.js renderer with solid Phong mesh, quad wireframe (gold) + triangle wireframe (amber) overlay, stats panel (input tri count, output quad count, output tri count, quad-ratio %), wireframe toggle. `feature_quad_remesh` LLM tool registered in `_TOOL_MODULES`. `llm_docs/feature_quad_remesh.md` documents schema + install + CLI flags. `FeatureView.jsx` inspector entry (bottom of FEATURE_KINDS). `FileTree.jsx` Grid3x3 icon for `quadmesh` kind. Instant Meshes is an optional dep: `brew install instant-meshes` (macOS); binary absent → HTTP 503 + friendly hint. |
| **Sketcher: Bezier curves** | ✅ shipped | New `bezier` entity kind in `.sketch`: `control_points` (array of point ids), `degree` (inferred). Degree 2 (quadratic, 3 pts) and degree 3 (cubic, 4 pts) supported; degree 4+ accepted but not recommended (use B-spline). UI: Bezier tool (key `Z`) — click control points, press Enter or double-click to commit; ghost preview renders the real curve as soon as ≥3 pts exist; control-point drag works via existing point-drag plumbing. Rendering: curve stroke + faint control-polygon hull in `SketchEntities`. Discretization: de Casteljau at 24 samples/degree (72 segs for cubic), chord error < 0.001·R. geom2: `tessellateBezier()` plugged into `buildAdjacency()`/`walkLoop()`/`findLoops()` for closed-loop extrude. Constraints: `bezier_tangent` (G1 direction, collinearity via `point_on_line_ppp`) and `bezier_g1` (G0+G1) in constraint palette and solver; **G2 deferred** — planegcs 1.1.x has no `CurvatureMatch` in its `push_primitive` API. sketchOps: trim/extend return unchanged with `console.warn` for Bezier (not well-defined without subdivision). Tests: 14 vitest + 6 pytest. |
| **Sketcher: symmetry over arbitrary construction line** | ✅ shipped | New `symmetric_over_line` constraint: `{ entity_a_id, entity_b_id, construction_line_id }`. Mirrors any entity (point, line, circle, arc, bezier, bspline) across a user-drawn construction line. Composite entities decomposed into multiple `p2p_symmetric_ppl` planegcs primitives via `decomposeSymmetric()`; circles/arcs also emit `equal_radius`. Arc mirroring swaps start↔end to preserve winding. UI: "Symmetric over construction line" button in the constraint palette — select 2 entities + 1 construction line → constraint added. Canvas glyph (⟺ badge) at the construction-line midpoint with hover-highlight. LLM doc updated in `sketch.md`. 12 vitest pass. |
| **Revit parity: IFC import** | ✅ shipped (Tier 1) | IfcOpenShell parses `.ifc` → `.bim` JSON. Tier 1: walls (`IfcWall`/`IfcWallStandardCase`), slabs (`IfcSlab`), spaces (`IfcSpace`), levels (`IfcBuildingStorey`), site (`IfcSite`). Importer: `kerf_bim.import_ifc` package (parser, walls, slabs, spaces, levels, sites, types). pyworker route `POST /import-ifc` (multipart `.ifc` → `{ bim_payload, stats, warnings }`). LLM tool `import_ifc` registered in plugin. `api.importIFC()` in `src/lib/api.js`. IFC placement chain handled via `ifcopenshell.util.placement.get_local_placement()`. IfcOpenShell absence → HTTP 503 with install hint. LLM doc `packages/kerf-bim/llm_docs/import_ifc.md`. 16 pytest tests (11 hermetic mocks + 5 live skipped without ifcopenshell). Tier 2 (families, schedules, views, curtain walls, MEP, openings) remains planned. |
| **Electrical: wiring + harness diagrams (`.wiring`)** | ✅ shipped | `packages/kerf-wiring/` plugin: `wireviz_runner.py` (GPLv3-gated), `POST /run-wireviz` pyworker route, `run_wireviz` LLM tool, `WiringView.jsx`, `.wiring` file kind + icon in `FileTree`. `api.runWireviz` in `src/lib/api.js`. `POST /api/projects/:pid/files/:fid/wiring/run` thin handler in kerf-api. 15 hermetic pytest tests; all green. WireViz is optional `[wireviz]` extra (GPLv3+) — hosted service uses subprocess isolation; local install is opt-in. |
| **Electrical: PLC structured text (`.plc.st`)** | ✅ shipped | **Tier 1 shipped**: `packages/kerf-plc/` plugin — `matiec_lint.py` subprocess wrapper (`iec2c` binary, 5 s timeout, graceful degradation when absent), `POST /lint-plc` pyworker route, `run_plc_lint` LLM tool. `PLCView.jsx` — Monaco editor with a custom `iec61131-st` language (full IEC 61131-3 keyword/type/operator list; registered via `registerIEC61131STLanguage`; custom `kerf-plc-dark` theme; lint debounce 600 ms → marker layer + bottom diagnostic panel; snapshot via `useImperativeHandle`). `.plc_st` file kind: migration `057_kind_plc_st.sql`, `SquareCode` icon (lime) in FileTree KIND_ROWS + KIND_ORDER, `api.lintPLC`. MATIEC is GPLv3 invoked as a subprocess — no in-process linking, no GPL taint. `llm_docs/plc.md` documents full ST schema + START/STOP latch example. 16 pytest + 29 vitest. **Tier 2 deferred** (v0.3+): `POST /run-plc-sim` simulated execution; OpenPLC runtime integration; LD/FBD graphical editors. |
| **Slicing: plane-section / cross-section** | ✅ shipped | `feature_section` wraps `BRepAlgoAPI_Section` (gated on the NURBS Phase 4 C1 probe at worker boot). Takes a target solid + `plane: { point, normal }` → returns the intersection edges as a `TopoDS_Compound`. Worker handler `opSection` is wired into both `evaluateTree` and `evaluateToFinalShape`. Python LLM tool `feature_section` (schema + validation + `run_feature_section`) registered in `kerf-cad-core`. New `.section` file kind: migration `053_kind_section.sql`, `Scissors` icon in FileTree, create-menu entry, FeatureView inspector entry under Modify (target solid picker + plane point/normal xyz fields). `SectionView.jsx` renders the edge compound as a 2D SVG with UV projection, pan/zoom, and a DXF export stub. Section-plane gumball (viewport drag) deferred to v0.3. |
| **Slicing: CNC layered slicing** | ✅ shipped | `feature_cam_layered` Python LLM tool stacks `BRepAlgoAPI_Section` calls at fixed Z (or X/Y) intervals. Schema: `{ target_solid_ref, z_step_mm, z_start_mm?, z_end_mm?, axis: "Z"|"X"|"Y" }`. Auto-detects bounding box when range is omitted. Emits `.cam.layered` document: `{ version, axis, z_step_mm, layers: [{z_mm, edges: [[[x0,y0],[x1,y1]],…]},…] }`. Registered in `kerf-cad-core` plugin (`_TOOL_MODULES`). New `cam_layered` file kind: migration `054_kind_cam_layered.sql`, `Layers` icon (teal) in FileTree, KIND_ROWS + KIND_ORDER entries. `LayeredCAMView` exported from `CAMView.jsx`: Z-slider scrubber between layers, per-layer 2-D SVG contour preview (same pan/zoom as SectionView), compact layer list, "Generate G-code from layers" button (POSTs to `/api/…/cam/layered/gcode` with safe-Z + plunge-feed, downloads `.nc`). G-code flow: each layer wrapped in existing `cam_contour` op, Z-step retracts (`G0 to safe-Z` / `G1 plunge`) emitted between layers by the backend. LLM doc `cam_layered.md`. Python tests: `test_cam_layered.py` (18 cases — validation, builder, fake-pool integration, OCC box tests skipped when OCC absent). JS tests: `camLayered.test.js` (FileTree wiring, LayeredCAMView API, migration 054, plugin registration, LLM doc). Decision: JS worker dispatch (`evaluateTree`/`evaluateToFinalShape` case for `cam_layered`) **not shipped in v0.2** — viewport scrubber uses the Python-produced `layers` array; worker dispatch deferred to v0.3 if real-time parameter preview is needed. |
| **Slicing: 3D-print G-code (FDM / SLA)** | ✅ shipped (Tier 1) | CuraEngine subprocess wrapper (AGPLv3 — called as separate process, hosted service stays MIT-compatible). Basic settings: layer_height, infill_density, perimeters, retraction, print_temperature, bed_temperature. `.print` file kind + `.print` config defines target mesh + settings dict. `feature_print_slice` LLM tool + pyworker `POST /run-print-slice` route. `PrintSliceView.jsx` showing per-layer G-code preview (text + 2D layer scrub of X-Y nozzle path). **Tier 2 deferred**: PrusaSlicer alternative, advanced settings (supports tuning, ironing, adaptive layers, brim/raft), G-code parser for time/material estimation. |
| **Workshop abuse-bound policy** | 🔮 planned | Per-file 100 MB · per-project total 500 MB · 100 files/project · 10 cover images · 20 publishes/user/month · per-account total Workshop storage bounded by tier storage cap (no separate allocation). Enforced at upload time + at publish-create endpoint. Email verification + manual review on first publish to bound multi-account spam. Matches Thingiverse + GitHub norms; bounds the storage-exposure hole on free + public Workshop. |
| **Public status page** | 🔮 planned | Real uptime monitoring (BetterStack / instatus / Cachet) with email + SMS alert routing on a `status.kerf.sh` subdomain. v0.1 footer doesn't link to a status page; users check `https://kerf.sh/healthz` or fly.io's metrics for now. |
...
| **Performance: server-side STEP pre-tessellation** | ✅ shipped | **Cloud-tier only**. `packages/kerf-tess/src/kerf_tess/worker.py::AutoTessWorker` wakes on PG `LISTEN/NOTIFY` `step_file_uploaded`, claims `step_tessellation_jobs` rows with `FOR UPDATE OF j SKIP LOCKED`, fetches the STEP blob, calls pyworker `POST /run-tess` (pythonOCC — wazero infeasible per Emscripten/WASI mismatch), stores the GLB at `meshes/step/<sha256>.glb` plus a `derived_artifacts(source_file_id, content_sha256, 'step_mesh')` row for content-hash idempotency, updates `files.mesh_storage_key`. Cloud-gated in `packages/kerf-workers/src/kerf_workers/runner.py`: `auto_tess_count = 1` only when `cloud_enabled and not local_mode`. NOTIFY fired from upload-finalise paths. Failure paths: timeout → error row, pyworker failure → error row (worker survives), stuck-running rows >10min re-queued on next loop, soft-fail on pyworker unavailable. Structured per-job log `auto_tess job_id=… file_id=… status=done|error sha256=… mesh_size=… duration_ms=…`. Migration `047_step_tess_input_spec.sql` adds `input_spec jsonb` + `content_sha256` columns + extends `derived_artifacts.derived_kind` check with `step_mesh`. Hermetic tests in `packages/kerf-tess/tests/test_auto_tess_worker.py` (12 cases: happy-path, idempotent cache hit, missing/empty storage, pyworker failure/timeout, deleted-file skip, input_spec passthrough, empty GLB). OSS local-install path stays browser-only — preserves single-binary brew/curl install. Big-STEP OSS users can pre-tessellate locally via `kerf-sdk`. |
...
| **Import: FreeCAD** | ✅ shipped (Tier 1 + 2) | Mechanical-CAD ingest. **Tier 1**: Part + PartDesign features → `.feature` + `.sketch` + `.assembly` (BRep lifted directly, no re-evaluation; all 19 Sketcher constraint types mapped). **Tier 2**: `Spreadsheet::Sheet` → `.equations` (aliased cells → named parameters), `TechDraw::DrawPage` + views → `.drawing` (projection direction → named view; position + scale preserved), `App::MaterialObject` → `.material` (density in kg/m³, Young's modulus in MPa, Poisson ratio, yield/UTS, thermal conductivity, color hex). Pure-Python parser (no FreeCAD install). 8 fixture files, 275 pytest tests. LLM doc: [`packages/kerf-imports/llm_docs/import_freecad.md`](./packages/kerf-imports/llm_docs/import_freecad.md). Tier 3 (Python-macro migration) and other workbenches (FEM/Path/BIM) out of scope. |
| ↳ **T1: `.FCStd` parser plumbing** | ✅ shipped | Pure-Python `.FCStd` → in-memory `FCStdDocument` representation: zip + `Document.xml` walk + `FileIncluded` byte loader for BRep blobs. New `packages/kerf-imports/src/kerf_imports/freecad/parser.py`. No FreeCAD dep — pythonOCC only. 41 unit tests; all green. Deps: none. |
| ↳ **T2: BRep-lift + `import_brep` worker op** | ✅ shipped | BRep-bytes → content-hashed lift via pythonocc `BRepTools::Read`. New `packages/kerf-imports/src/kerf_imports/freecad/brep_importer.py`: `lift_brep_blob`, `build_feature_tree`, `ImportResult`, `BRepLiftError`. One `import_brep` node per `PartDesign::Body`; placeholder `brep:<sha256>` asset_id replaced by T7. 14 pytest tests; all green. Awaits T7 for route wiring + asset upload. |
| ↳ **T3: Sketch translator (Sketcher → `.sketch`)** | ✅ shipped | Translate FreeCAD `Geometry` + `Constraints` property lists → Kerf `.sketch` JSON. Hand-curated constraint mapping table (all 19 constraint types; Coincident, Horizontal/Vertical, Distance{X,Y}, Angle, Tangent, Equal, PointOnObject, Symmetric, Block, Diameter mapped; SnellsLaw/Weight/InternalAlignment dropped with warnings). New `freecad/sketch.py`. 40 pytest tests; all green. Deps: T1. |
| ↳ **T4: PartDesign feature-tree metadata capture** | ✅ shipped | Walk PartDesign features, emit `.feature` JSON: `import_brep` node first + read-only metadata nodes per Pad/Pocket/Fillet/Chamfer/Pattern etc. with `freecad_ref` provenance + `read_only: true`. New `freecad/features.py`. 21 pytest tests; all green. Deps: T1, T2, T3. |
| ↳ **T5: Multi-Body `.FCStd` → `.assembly`** | ✅ shipped | Detect multi-Body documents, emit `.assembly` with one Component per Body; `Placement` quaternion → 4×4 matrix transform. Single-body docs skip assembly. New `freecad/assembly.py`. 21 pytest tests; all green. Deps: T1, T4. |
| ↳ **T6: Pyworker route `POST /import-freecad-project`** | ✅ shipped | Full T1+T3+T4+T5 pipeline in `freecad/route.py` (replaces legacy stub). Returns `{created_files, stats, warnings, import_folder}`. Legacy `/import-freecad` kept for backwards compat. 26 pytest tests; all green. Deps: T1, T3, T4, T5. |
| ↳ **T7: LLM tool `import_freecad_project`** | ✅ shipped | New `packages/kerf-imports/src/kerf_imports/tools/import_freecad.py`. Same shape as `import_3dm`: fetch blob, call pyworker `/import-freecad-project`, walk response, insert files in PG. `mode: "project" \| "library"` flag. Registered in `plugin.py`. `api.importFreecadProject` stub in `src/lib/api.js` replaced with real `POST /api/projects/:id/imports/freecad` call (removes `FREECAD_T7_PENDING`). 17 pytest tests; all green. Deps: T6. |
| ↳ **T8: Integration test + `.FCStd` fixtures** | ✅ shipped | Five minimal `.FCStd` fixtures committed (`single_pad`, `pad_and_pocket`, `two_bodies`, `sketch_constraints`, `unsupported_constraints`) built via `scripts/generate_freecad_fixtures.py` (pure-Python, no `freecadcmd` needed). E2e test `test_freecad_e2e.py` uploads each fixture through the route, asserts file tree, constraint counts, warning counts, and assembly transform. 34 pytest tests; all green. Fixtures README documents regen procedure. Deps: T7. |
| ↳ **T9: Frontend "Import FreeCAD" picker (optional)** | ✅ shipped | `src/components/FreeCADImport.jsx` — `isFCStdFile()` predicate, `<FreeCADImportDialog>` (drag-drop drop-zone + file picker + upload via `uploadAssetChunked` + progress card), `<FreeCADImportButton>`. Wired into `FileTree` CreateMenu ("Import FreeCAD" row + `.FCStd` drag-drop onto tree background) and `Projects.jsx` toolbar ("Import FreeCAD" button → create blank project → open dialog). `api.importFreecadProject` stub raises `FREECAD_T7_PENDING` error code so the UI surfaces an honest "awaiting T7 backend wiring" message; T7 swaps the stub for the real `POST /api/projects/:id/imports/freecad` call in one line. 16 vitest tests; all green. |
...
| **LLM tool consolidation (doc-search + small fixed surface)** | ✅ shipped | ~30 domain-specific tools collapsed into a small fixed surface (file ops, object ops, BOM, validation, four `create_*` scaffolders) plus `search_kerf_docs` over an embedded markdown corpus under `packages/kerf-chat/llm_docs/` (and per-plugin `llm_docs/` dirs). The model reads the relevant doc page and edits the file's JSON / TSX directly via `write_file` / `edit_file`. Adding a new domain is a markdown change, not a code change. |
...
| **Rhino parity: 3DM file format** | ✅ shipped | `packages/kerf-imports/src/kerf_imports/rhino3dm_route.py` + `packages/kerf-imports/src/kerf_imports/tools/import_3dm.py` + `src/lib/rhino3dm.js` + import_3dm.md shipped. |
| **Rhino parity: SubD modeling** | ✅ shipped | `packages/kerf-imports/src/kerf_imports/tools/subd.py` + `src/lib/subd.js` + `.subd` file kind + Catmull-Clark + 044 migration shipped. |
...
| **Rhino parity: mesh tools** | ✅ shipped | `packages/kerf-imports/src/kerf_imports/tools/mesh.py` + `src/lib/meshTools.js` + 044 migration shipped. |
| **Rhino parity: parametric graph (Grasshopper-equivalent)** | ✅ shipped | `packages/kerf-imports/src/kerf_imports/tools/graph.py` + `src/lib/graph.js` + `graphOps.js` + `.graph` file kind + `GraphEditor.jsx` shipped. |
| **Rhino parity: render-quality output** | ✅ shipped | `packages/kerf-render/src/kerf_render/routes.py` + `packages/kerf-render/src/kerf_render/tools.py` + `src/lib/render.js` + `.render` file kind shipped. |
| **Rhino parity: drafting completeness** | ✅ shipped | `src/lib/draftingComplete.js` + drafting completeness shipped (hatch, leaders, rich text, dim chains). |
| **Rhino parity: curve depth** | ✅ shipped | `src/lib/curveOps.js` + `packages/kerf-imports/src/kerf_imports/tools/curve_ops.py` shipped (12 ops). |
| **Revit parity: categories + hosted refs** | ✅ shipped | `src/lib/bimCategories.js` + `packages/kerf-bim/src/kerf_bim/tools/bim_categories.py` shipped. |
| **Revit parity: type vs instance params** | ✅ shipped | Covered by family.py + element_types.py. |
| **Revit parity: `.family.json` parametric components** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/family.py` + `src/lib/family.js` + 037 migration shipped. |
| **Revit parity: `.schedule.json` live queries** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/schedule.py` + `src/lib/schedule.js` + 038 migration shipped. |
| **Revit parity: `.view.json` derived views** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/view.py` + `src/lib/view.js` + 039 migration shipped. |
| **Revit parity: `.sheet.json` print-ready layouts** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/sheet.py` + `src/lib/sheet.js` + 039 migration shipped. |
| **Revit parity: phasing + filters** | ✅ shipped | `src/lib/phasing.js` + `src/lib/viewFilters.js` shipped. |
| **Revit parity: stairs + railings** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/stairs.py` + `railings.py` + 041 migration shipped. |
| **Revit parity: MEP routing** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/mep.py` + `src/lib/mep.js` + 040 migration shipped. |
| **Revit parity: curtain wall** | ✅ shipped | `packages/kerf-bim/src/kerf_bim/tools/curtain_wall.py` + `src/lib/curtainWall.js` + 042 migration shipped. |
...
| **Sketch → JSCAD workflow (mesh-side analog of `.feature`)** | ✅ shipped | Constrained 2D `.sketch` with dimensions → AI-generated `.jscad` 3D part. Sketch stays the source of truth — editing a sketch dimension reflows the 3D (incl. cross-file dep graph: sketch → jscad → assembly). All 5 sub-tasks shipped: jscad.md doc rewrite, `extrude_sketch_to_jscad` LLM tool, sketch-import error surfacing, reactive re-eval, SketchView "Build 3D" dropdown + file-tree backlink chip. See [docs/plans/sketch-to-jscad.md](docs/plans/sketch-to-jscad.md). |
...
| ↳ Reactive re-eval when an imported sketch changes | ✅ shipped | `updateSketch` now calls `_reEvalJscadForSketch` after persisting; `sketch_*` LLM tools added to `FILE_MUTATING_TOOLS` so `sendMessage` triggers `loadFileForEditor` (which re-runs `runJscad`). `SKETCH_IMPORT_RE` exported from `jscadRunner.js` for shared parse. Vitest coverage in `src/__tests__/jscadReactiveReEval.test.js`. Cross-file graph invalidation (assembly → jscad → sketch) deferred. |
| ↳ "Build 3D" SketchView affordance + file-tree backlink | ✅ shipped | "Build 3D ▾" dropdown in the SketchView toolbar (amber, only shown when sketch has ≥ 1 closed loop): Extrude → (height_mm, default 10 mm) and Revolve → (angle_deg, default 360°). Small modal collects params + default output filename (`<sketch-name>.jscad`). `createJscadFromSketch` action in `workspace.js` scaffolds the canonical JSCAD source and opens the new file. File-tree backlink chip ("← bracket.sketch") rendered on `.jscad` rows via `jscadSketchLinks` Map (populated by `_rebuildJscadSketchLinks` on file load/create). Vitest in `src/__tests__/buildThreeD.test.js`. |
| **FreeCAD parity: Draft workbench (2D CAD)** | ✅ shipped | `packages/kerf-imports/src/kerf_imports/tools/draft.py` + `src/lib/draft.js` + 045 migration shipped. |
| **FreeCAD parity: inspection / model comparison** | ✅ shipped | `packages/kerf-imports/src/kerf_imports/tools/inspection.py` + `src/lib/modelCompare.js` shipped. |
| **KiCad parity: hierarchical schematics** | ✅ shipped | `packages/kerf-electronics/src/kerf_electronics/tools/hier_schematic.py` + `src/lib/hierSchematic.js` shipped. |
| **KiCad parity: buses + differential pairs** | ✅ shipped | `packages/kerf-electronics/src/kerf_electronics/tools/buses.py` + `src/lib/buses.js` shipped. |
| **KiCad parity: net classes + design rules** | ✅ shipped | `packages/kerf-electronics/src/kerf_electronics/tools/net_classes.py` + `src/lib/netClasses.js` shipped. |
| **KiCad parity: length tuning + match** | ✅ shipped | `packages/kerf-electronics/src/kerf_electronics/tools/length_tuning.py` + `src/lib/lengthTuning.js` shipped. |
| **KiCad parity: via stitching + teardrops** | ✅ shipped | `packages/kerf-electronics/src/kerf_electronics/tools/via_stitching.py` + `src/lib/viaStitching.js` shipped. |
| **KiCad parity: push-pull interactive routing** | ✅ shipped | `packages/kerf-electronics/src/kerf_electronics/tools/shove_router.py` + `src/lib/shoveRouter.js` shipped. |
...
| **Persistent face naming (topological IDs)** | 🚧 in flight | Replace positional `target_face_id: N` with stable, human-readable names (`Pad-A.TopCap`, `Pad-A.Side.seg-3`) that survive upstream sketch / feature edits. Sketch-anchored primary + topological-signature fallback; dual-write `target_face_name` + legacy `target_face_id` for backwards compat. Fixes silent-renumber bug across `feature_push_pull`, `feature_cut_from_sketch`, `feature_boss_with_draft`, `feature_fillet`, `feature_chamfer`, pattern features, mate refs. Design doc: [`docs/plans/persistent-face-naming.md`](./docs/plans/persistent-face-naming.md). ~5.5 sonnet-agent-days (T1–T7). |
| ↳ **T1: Worker face-name emitter (sketch-anchored, base ops)** | ✅ shipped | `src/lib/faceNaming.js` — pure-JS helpers (`classifyFaceForExtrude`, `classifyFaceForPocket`, `classifyFaceForRevolve`, `buildFaceNamesForExtrude`, `buildFaceNamesForRevolve`, `topoHash`). `src/lib/occtWorker.js` extended: `opPad`/`opPocket`/`opRevolve`/`opBossWithDraft` emit `faceNames: Record<faceIndex,name>` in the mesh payload alongside legacy `faceIds`. Both `evaluateTree` and `evaluateToFinalShape` dispatch tables updated (dormant-node-bug fix). `face_outline` response includes `faceNames`. 39 unit tests in `src/__tests__/faceNaming.test.js` (topoHash determinism + stability, cap detection, side-face → sketch-entity mapping, pocket Inner. prefix, fallback hash, collision resolution). |
| ↳ **T2: Role taxonomy + carry-over for fillet/chamfer/shell/cut/push_pull** | ✅ shipped | `src/lib/faceNaming.js` extended: `carryForward(inputFaceNames, outputFaceIndex, modifiedMap)` carry-over helper; `nameOpOutput(opKind, oldFaceNames, newFaces, modifiedMap, opMeta)` orchestrator with deterministic roles per op (`Fillet`, `Chamfer`, `Wall.<hash>`, `CutFloor`, `CutSide.<id>`, `PushPullCap`, `PushPullSide.<hash>`, `Original.<prior>` carry-over). `src/lib/occtWorker.js` extended: `extractModifiedMap(oc, builder, inputShape, outputShape)` extracts OCCT Modified/Generated/IsDeleted maps; helper closures `makeFilletChamferNamer`, `makeShellNamer`, `makeCutFromSketchNamer`, `makePushPullNamer`; both `evaluateTree` and `evaluateToFinalShape` dispatch tables updated for `fillet`/`chamfer`/`shell`/`push_pull`/`cut_from_sketch` with builderRef capture. Op functions (`opFillet`, `opChamfer`, `opShell`, `opCutFromSketch`, `opPushPull`) accept optional `builderRef` parameter (backwards-compatible). 45 new unit tests added to `src/__tests__/faceNaming.test.js` covering `carryForward` semantics + `nameOpOutput` for all 5 op kinds. OCCT binding gap: `Modified/IsDeleted/Generated` list-iteration pattern (TopTools_ListOfShape iterator) may not be bound in all opencascade.js builds — `extractModifiedMap` falls back to treating all uncovered output faces as generated when iteration fails. |
...
- **LLM tool:** `set_parameter({ name, value | expression, unit?, description? })`.
  ...
  `packages/kerf-chat/llm_docs/equations.md`.
...
- **LLM tools:** `add_configuration({ file, name, overrides })`,
  ...
  `packages/kerf-chat/llm_docs/configurations.md`.
...
- **LLM tool:** single `fem_run({ source_file, materials, fixtures,
  ...
  `packages/kerf-chat/llm_docs/fem.md` per the consolidated tool pattern.
...
— minimal v1, doc page in `packages/kerf-chat/llm_docs/`, single LLM
...
`packages/kerf-electronics/src/kerf_electronics/routes.py` (pour endpoint) that returns the filled polygon (polygon-with-
...
- **File-kind = data.** Every Rhino concept becomes a JSON file kind
  with a schema doc in the relevant plugin's `llm_docs/<kind>.md` (most cross-cutting docs in `packages/kerf-chat/llm_docs/`). No exceptions.
...
### Phase 3: server-side STEP pre-tessellation — ✅ shipped
...
- **D. Python sidecar via pythonOCC (chosen).** Top-level `pyworker/`
  service exposes `POST /tessellate-step` (and all other heavy-compute
  routes). Existing `step_tessellation_jobs` row + `auto_tess_worker`
  + mesh GLB output all survive — only the sidecar invocation changes
  (HTTP to pyworker instead of subprocess). pythonOCC is the full OCCT binding
...
**S3 — Server-side pre-tessellation (✅ shipped)**

Big STEPs arrive as `.glb` instead of raw B-rep. `AutoTessWorker` in
`packages/kerf-tess/src/kerf_tess/worker.py` wakes on PG NOTIFY, calls
the pyworker `/run-tess` route (pythonOCC), caches the mesh in
`derived_artifacts` keyed by `(file_id, content_sha256, 'step_mesh')`, and
writes `files.mesh_storage_key`. See Performance Phase 3 above.
...
| Stateless object-storage git backend | ✅ shipped | `S3GitStorer` (`packages/kerf-core/src/kerf_core/storage/git_storer.py`) bulk-syncs bare pygit2 repos to/from R2/S3. Objects-before-refs upload order + `_marker` ETag concurrency check + batch orphan delete. Wired into the cloud-only `git_push` / `git_pull` routes in `packages/kerf-cloud/src/kerf_cloud/routes.py` behind an `isinstance(storage, S3Storage)` guard so the OSS filesystem path is unaffected. 6 hermetic moto integration tests in `packages/kerf-core/tests/test_git_storer.py` cover the full round-trip + repack + race detection. Docs: `docs/cloud.md` § S3 Git Storer. |
| `pyworker` service (Python sidecar) | ✅ shipped | Per-plugin FastAPI routes, assembled by the plugin loader. Routes shipped: `/tessellate-step` (`kerf-tess`), `/run-mates` (`kerf-mates`), `/run-cam` (`kerf-cam`), `/run-rf` + `/autoroute-circuit` (`kerf-electronics`), `/run-fem` (`kerf-fem` — **FEniCSx primary; CalculiX as documented second-solver behind same route**), `/run-topo` (`kerf-topo`), `/compile-ifc` + `/compile-bim` (`kerf-bim`), `/import-kicad` + `/import-kicad-library` (`kerf-electronics`), `/import-freecad` (`kerf-imports`), `/run-spice` (`kerf-electronics`), `/import-rhino3dm` (`kerf-imports`), `/render` (`kerf-render`). One Python runtime hosts every heavy-compute server-side feature — amortizes ops cost. Cloud-tier wires it via `packages/kerf-workers/src/kerf_workers/runner.py`; OSS local-install path uses the per-plugin routes directly when installed (otherwise browser-side fallback). Preserves [[local_install_model]]. |
| API tokens (cloud) for `kerf-sdk` auth | ✅ shipped | `api_tokens` table (migration `025_api_tokens.sql`) with `scopes jsonb`, `last_used_at`, soft-revoke; `packages/kerf-core/src/kerf_core/db/queries/api_tokens.py` query layer; `POST/GET/DELETE /api/api-tokens` endpoints in `packages/kerf-auth/src/kerf_auth/routes.py` (workspace context via `?workspace_slug=` query param for JWT-auth callers); `WorkspaceSettings.jsx` API Tokens card with issue/list/revoke flow. Workspace-scoped opaque tokens (`kerf_sk_` + 32 chars, DB-lookup not JWT). Out of scope v1: browser device flow, scope-narrowing UI, full audit log, multi-workspace tokens. |
...
The code structure mirrors this roadmap: `packages/` is the OSS Python monorepo (MIT-licensed plugins), `cloud/` is cloud-tier Python (proprietary), `src/` is OSS frontend, `src/cloud/` is cloud frontend. New features land in the right tree based on their license.
