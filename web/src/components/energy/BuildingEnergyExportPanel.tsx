...
const CZ_DEFAULTS: Record<string, ClimateZoneDefaults> = {
...
type NumericZoneField = 'floor_area_m2' | 'ceiling_height_m' | 'window_area_m2' |
  'wall_u_value' | 'window_u_value' | 'window_shgc' | 'roof_u_value' |
  'infiltration_ach' | 'lighting_w_m2' | 'equipment_w_m2' | 'occupancy_people' |
...
function ZoneEditor({ zone, onChange, onRemove, idx }: {
  ...
  const set = (key: keyof Zone, val: Zone[keyof Zone]) => onChange({ ...zone, [key]: val })
      ...
      {open && (
        <div className="px-3 pb-3 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            ] as [string, NumericZoneField, number][]).map(([label, key, step]) => (
              <div key={key}>
...
export default function BuildingEnergyExportPanel({ projectId }: { projectId: string }) {
  const [zones, setZones] = useState<Zone[]>([defaultZone(1)])
  ...
  const updateZone = (idx: number, z: Zone) => {
    const next = [...zones]
    next[idx] = z
    setZones(next)
  }
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      // migration. Real bug, reported not fixed — same defect as
      // src/components/optics/DaylightingPanel.tsx's identical call.
