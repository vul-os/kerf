...
function makeSimResult({ variables = {}, rung_results = [], tick = 1, error }: { variables?: Record<string, unknown>; rung_results?: unknown[]; tick?: number; error?: unknown } = {}) {
  const result: { variables: Record<string, unknown>; rung_results: unknown[]; tick: number; error?: unknown } = { variables, rung_results, tick }
