...
| **Revit parity: IFC import** | 🔮 planned | Currently we export IFC4; import is the reverse. IfcOpenShell parses `.ifc` → `.bim` DSL + level/wall/slab nodes. Tier 1: walls/slabs/spaces/levels only (matches our DSL). Tier 2: families + schedules + views. New pyworker route `POST /import-ifc`. |
