...
| ↳ **Lua SDK (LuaRocks: kerf-sdk)** | 📋 planned | `packages/kerf-sdk-lua/`. Pure-Lua JSON-RPC 2.0 client over `lua-socket` / `lua-http`. `kerf.from_env()` + `kerf:connect(opts)`. Namespaced via tables: `k.files:list{...}`, `k.equations:read{...}`. `.rockspec` for LuaRocks publish on `sdk-lua-v*` tags. Same wire format. Target user: existing CAD plugins ecosystems (Blender, Rhino) where Lua is the embedded script host. |
...
| **Rhino parity: Quad remesher** | 🔮 planned | Quad-dominant remeshing — distinct from the existing triangle `mesh.remesh` op. Open-source path: Instant Meshes (MIT, ~10k LOC C++) — wasm or native subprocess. `feature_quad_remesh` op on a triangle mesh → quad-dominant mesh. Useful for SubD prep + downstream FEM. Plug-in territory. |
