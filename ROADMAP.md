...
| ↳ **Go SDK (`github.com/kerf-sh/kerf-sdk-go`)** | 📋 planned | `packages/kerf-sdk-go/`. Standard-library `net/http` JSON-RPC 2.0 client. `kerf.FromEnv()` + `kerf.New(token, url)`. Namespaced sub-packages: `kerf/files`, `kerf/equations`, `kerf/configurations`, `kerf/revisions`, `kerf/docs`. Errors wrap `*kerf.Error` with code + message + data. `.github/workflows/publish.yml` Go-module tag-push on `sdk-go-v*` (Go modules use the repo path, no separate registry). Same wire format. |
