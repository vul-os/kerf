...
| ↳ **T9: Frontend "Import FreeCAD" picker (optional)** | 🔮 planned | `src/components/ImportFreecadDialog.jsx` — `.FCStd` file-type pattern + drop + project-creation flow that invokes the LLM tool; honest framing of the read-only feature-tree for the user. Out of plugin scope; the LLM tool alone gives the chat-driven path. Deps: T7. |
