...
## 2026-08-10 SAST — Answered: is today's parity page as good as the old React compare page? (found + read the old source)

**Context:** Four prior agents were asked whether `site/parity.html` (static, generated by
`scripts/build-parity-site.mjs` from `web/public/compare-manifest.json`, 1,397 rows / 57 tools) is
as good as, or better than, the old React-based compare page. All four stalled before actually
reading the old source and reporting a comparison. This fire found it, read it in full, and is
recording the answer here so a fifth agent never has to redo this.

**Where the old page lives (git history, not on any live branch):**
Found via `git log --oneline --all -- '*ompare*' '*arity*' '*ricing*'` and
`git log --all --diff-filter=D --name-only | grep -iE 'compare|parity|pricing'`, then read with
`git show <sha>:<path>`. The React compare system was several files, last present as JSX/TSX at
commit `bb0761ed^` (`d8e7818`) before a same-day TS migration, and deleted outright in
`290d3318` (frontend moved to `web/`, compare pages migrated to markdown by then) / fully gone
after `821b566b`:
  - `src/routes/compare/CompareLanding.tsx` — the `/compare` hub (search + category pills + card grid + by-domain grid)
  - `src/routes/compare/CompareByDomain.tsx` — `/compare/by-domain/:slug`, one cross-tool pivot table per D-domain
  - `src/components/CompareFeatureMatrix.jsx` — embedded in each per-tool `/compare/:slug` page, D1–D14 collapsible sections
  - `src/components/CompareCardGrid.tsx`, `src/components/CompareCategoryPill.tsx` — landing-page pieces
  - `src/lib/compareFeatures.js`/`.ts` — `DOMAIN_META` (14 domains), `STATUS_META` (yes/partial/paid/no/unknown), `TONE_CLASSES`
  - `src/routes/Pricing.jsx` (deleted in `6f0e66ba`, "remove billing entirely") — the pricing tiers the task description referenced: Free $0 / Studio $9 / Pro $29, a separate page linked from nav, not embedded in the matrix itself.

**The concrete comparison (old React vs. today's `site/parity.html` + `site/parity/<slug>.html`), as read from source, before this fire's edits:**

*Interaction* — Old had: a live search box on the landing page (substring match on competitor + slug + tagline via `compareSearch.js`), category filter pills ("All" + 7 cats), per-tool pages with D1–D14 sections as native `<details>` showing "N of M matched", and a separate by-domain cross-tool pivot table (`CompareByDomain.tsx`) with the Feature column `position:sticky; left:0` (real column pinning) while more CAD-tool columns scrolled underneath — but no sticky *header row* on that same table. Today's baseline had: sticky header row (`thead th { position:sticky; top:0 }`) already present on every table, but no search, no category filter, no expand/collapse, no column pinning, and no by-domain cross-tool pivot page at all (a per-tool page only ever compares Kerf vs. one competitor, never multiple competitors side by side). Net: old page had richer interaction on both axes (row-pinning old had, header-pinning new had); neither had sorting.

*Information design* — Old grouped every per-tool matrix into 14 named, collapsible D-domain sections with a live "N of M matched" count per section, plus a responsive mobile fallback (stacked cards instead of a table under `sm` breakpoint) and an explicit colour+symbol+label legend (✓/~/$/✗/?) on the by-domain page. Today's baseline flattened everything into one long table per tool with domain shown only as small grey text under the feature name (no section grouping, no mobile-specific layout — just horizontal scroll), and a legend only at the bottom of each detail page (present, roughly equivalent). partial/full/absent representation: both use the same status vocabulary (yes/partial/paid/no/unknown) — today's `chip` classes are a direct rename of the old `STATUS_META`/`TONE_CLASSES` (same 5 states, same colour logic), so that part carried over faithfully during the markdown migration.

*Visual polish* — Comparable overall; both are dark-theme, monospace-accented, card/table-based designs from the same design language (today's page literally reuses `site/index.html`'s CSS custom properties). Old had per-category accent colours on cards/pills (14 distinct domain accents, 7 category accents) and a coverage-bar-free card (just tagline + arrow). Today's baseline card grid *adds* something old didn't have: a proportional coverage bar (`.bar` — yes/partial/paid/no segments) with a text equivalent underneath, which is a real improvement over the old plain-tagline card. Today's fairness framing ("Read this first" intro, "rows Kerf does not fully cover" stat, "Where Kerf falls short" table, the honest-percentage footer note) has **no equivalent in the old page at all** — the old page never surfaced Kerf's own gaps up front; that's new and is the single biggest win of the current page over the old one, non-negotiable per the task brief.
