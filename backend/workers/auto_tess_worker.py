...
Canonical source: kerf-tess/src/kerf_tess/worker.py

This module attempts to import from the kerf_tess plugin package.
When kerf_tess is not installed (e.g. in the legacy backend-only env),
it falls back to the original inline implementation so that existing
backend code and tests continue to work without modification.
...
``settings.local_mode`` is True or ``settings.cloud_enabled`` is False.
"""

from __future__ import annotations

try:
    from kerf_tess.worker import (  # noqa: F401
        AutoTessWorker,
        notify_step_uploaded,
        _read_storage,
        _write_storage,
        _TessHTTPDriver,
    )
    # Successfully imported from plugin package; nothing more to do.

except ImportError:
    # ── Inline fallback (kerf_tess not installed) ─────────────────────────────
    import asyncio
    import hashlib
    import json
    import logging
    ...
    class AutoTessWorker(BaseWorker):
        ...
        async def _process(self, *, job_id, file_id, storage_key: str, spec: TessInputSpec, t0: float) -> None:
            ...
            if cached:
                await self._mark_done(
                    ...
                    content_sha256=sha256_hex, mesh_storage_key=cached,
                )
            ...
            try:
                await self._persist_artifact(file_id=file_id, content_sha256=sha256_hex, payload=glb_bytes)
            except Exception:
                logger.exception("auto_tess: derived_artifact upsert failed (job=%s)", job_id)

            await self._mark_done(
                job_id=job_id, file_id=file_id,
                content_sha256=sha256_hex, mesh_storage_key=mesh_key,
            )
        ...
        async def _persist_artifact(self, *, file_id, content_sha256: str, payload: bytes) -> None:
            async with self.pool.acquire() as conn:
                await conn.execute(
                    ...
                    """,
                    file_id, content_sha256, payload, len(payload),
                )

        async def _mark_done(self, *, job_id, file_id, content_sha256: str, mesh_storage_key: str) -> None:
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    await conn.execute(
                        """
                        ...
                        WHERE id = $1
                        """,
                        job_id, mesh_storage_key, content_sha256,
                    )
                    await conn.execute(
                        "UPDATE files SET mesh_storage_key = $2 WHERE id = $1",
                        file_id, mesh_storage_key,
                    )
