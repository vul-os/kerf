...
| ↳ **Rust SDK (crates.io: kerf-sdk)** | 📋 planned | `packages/kerf-sdk-rs/`. `reqwest`-based JSON-RPC 2.0 client, async via `tokio`. Namespaced struct methods mirroring Python + TS SDKs. `from_env()` reads `KERF_API_TOKEN` + `KERF_API_URL`. `KerfError` enum with explicit variants per RPC error code. `.github/workflows/publish.yml` crates.io publish on `sdk-rs-v*` tags. Same wire format as Python + TS — all three target `POST /v1/rpc`. Auto-doc via `cargo doc`. |
...
| **Electrical: PLC structured text (`.plc.st`)** | 🔮 planned | IEC 61131-3 Structured Text — Pascal-shaped text DSL for ladder logic / function blocks. Open-source runtime: [OpenPLC](https://www.openplcproject.com/) (GPLv3) — compiles ST → MATIEC → C/Cython. Tier 1: syntax-highlighted editor + offline lint via MATIEC parser. Tier 2: optional `POST /run-plc-sim` for simulated execution against synthetic inputs. Companion to `.circuit.tsx` on the electronics persona. |
