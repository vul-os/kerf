...
| **Performance: server-side STEP pre-tessellation** | ✅ shipped | **Cloud-tier only**. `backend/workers/auto_tess_worker.AutoTessWorker` wakes on PG `LISTEN/NOTIFY` `step_file_uploaded`, claims `step_tessellation_jobs` rows with `FOR UPDATE OF j SKIP LOCKED`, fetches the STEP blob, calls pyworker `POST /run-tess` (occt-import-js Node sidecar — wazero infeasible per Emscripten/WASI mismatch, pythonOCC route also wired for future), stores the GLB at `meshes/step/<sha256>.glb` plus a `derived_artifacts(source_file_id, content_sha256, 'step_mesh')` row for content-hash idempotency, updates `files.mesh_storage_key`. Cloud-gated in `main.py`: `auto_tess_count = 1` only when `cloud_enabled and not local_mode`. NOTIFY fired from `routes/api.py` upload-finalise paths (small + step-ref). Failure paths: timeout → error row, pyworker failure → error row (worker survives), stuck-running rows >10min re-queued on next loop, soft-fail on pyworker unavailable. Structured per-job log `auto_tess job_id=… file_id=… status=done|error sha256=… mesh_size=… duration_ms=…`. Migration `047_step_tess_input_spec.sql` adds `input_spec jsonb` + `content_sha256` columns + extends `derived_artifacts.derived_kind` check with `step_mesh`. Hermetic tests in `backend/tests/test_auto_tess_worker.py` (12 cases: happy-path, idempotent cache hit, missing/empty storage, pyworker failure/timeout, deleted-file skip, input_spec passthrough, empty GLB). OSS local-install path stays browser-only — preserves single-binary brew/curl install. Big-STEP OSS users can pre-tessellate locally via `kerf-sdk`. |
...
| `pyworker` service (Python sidecar) | ✅ shipped | Top-level `pyworker/` (FastAPI). Routes shipped: `/tessellate-step` (pythonOCC via occt-import-js sidecar), `/run-mates` (python-solvespace), `/run-cam` (OpenCAMlib + pythonOCC), `/run-rf` (scikit-rf), `/run-fem` (**FEniCSx primary; CalculiX as documented second-solver behind same route**), `/run-topo` (FEniCSx SIMP), `/compile-ifc` + `/compile-bim` (IfcOpenShell), `/import-kicad` + `/import-kicad-library` (kiutils), `/import-freecad` (FreeCAD Python module), `/run-spice` (ngspice), `/autoroute-circuit` (FreeRouting), `/import-rhino3dm`, `/render`. One Python runtime hosts every heavy-compute server-side feature — amortizes ops cost. Cloud-tier wires it via the `auto_tess_worker` + similar workers; OSS local-install path uses pyworker directly when user installs it (otherwise the browser-side fallback runs). Preserves [[local_install_model]]. |
| API tokens (cloud) for `kerf-sdk` auth | ✅ shipped | `api_tokens` table (migration `025_api_tokens.sql`) with `scopes jsonb`, `last_used_at`, soft-revoke; `backend/db/queries/api_tokens.py` query layer; `POST/GET/DELETE /api/api-tokens` endpoints in `backend/routes/auth.py` (workspace context via `?workspace_slug=` query param for JWT-auth callers); `WorkspaceSettings.jsx` API Tokens card with issue/list/revoke flow. Workspace-scoped opaque tokens (`kerf_sk_` + 32 chars, DB-lookup not JWT). Out of scope v1: browser device flow, scope-narrowing UI, full audit log, multi-workspace tokens. |
