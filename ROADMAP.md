...
| **CAM toolpath generation** | 🚧 in flight | OpenCAMlib (LGPL 2.1); 2.5D ops (face/contour/pocket/drill/profile); G-code with selectable post (LinuxCNC/GRBL/Mach3/Fanuc); `.cam` file kind wired: `POST/GET /api/.../cam` + `/cam/status`, `cam_run` / `cam_job_status` LLM tools, `CAMView.jsx`, `llm_docs/cam.md`, opencamlib+pythonOCC try/except gated with mock fallback. STEP→STL via pythonOCC (`STEPControl_Reader` + `BRepMesh_IncrementalMesh` + `StlAPI_Writer`, 0.1 mm linear deflection) wired into `pyworker/routes/cam.py`; `ocl.STLSurf` populated from parsed ASCII STL; `PathDropCutter` raster/perimeter/drill paths driven by real surface bounds; pytest `backend/tests/test_cam_step.py` asserts CL-point XY within cube footprint (skipped when pythonOCC/opencamlib absent). Remaining: contour/pocket boundary extraction from B-rep faces (currently bounding-box fallback), CAM2 3D parallel+waterline, CAM3 lathe/5-axis. |
| **Topology optimization** | 🚧 in flight | Density-field SIMP via FEniCSx; `dolfinx`/`gmsh`/pythonOCC imports gated with try/except so pyworker boots without them. `.topo` file kind wired; `topo_run` LLM tool; pyworker `POST /run-topo` — when dolfinx+gmsh are available meshes the real `.feature` STEP via `gmsh.model.occ.importShapes` + `dolfinx.io.gmshio.read_from_msh`, runs full SIMP loop (Heaviside filter, OC update, Heaviside projection) with loads/BCs from `.topo` spec (face_tag-indexed Dirichlet + Neumann); when Gmsh absent falls back to unit-cube mesh; marching-cubes (skimage) + pythonOCC BRep sewing exports optimized density ≥ 0.5 threshold as faceted STEP stored as new `kind='step'` file with `output_mesh_file_id` populated; when not available returns `ENGINE_PENDING_WARNING` sentinel. `TopoView.jsx` renders spec params, results, `DensityFieldHeatmap` (SVG X-Y projection of density field), `DensityMeshViewer` (GLB when output_mesh_file_id is set); wired in `Editor.jsx` for `.topo` kind. `llm_docs/topo.md` documents full schema + BC/load spec + face_tag convention. `pyworker/tests/test_topo_phase2.py` covers Gmsh meshing, SIMP non-triviality, marching-cubes→STEP round-trip (skipped without deps). Remaining: NURBS STEP reconstruction (faceted shell today), contour smoothing, multi-body optimization. |
