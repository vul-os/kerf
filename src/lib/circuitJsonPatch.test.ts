// circuitJsonPatch.test.ts — vitest tests for circuitJsonPatch.ts helpers.
//
...
import {
  addFootprint as addFootprintImpl,
  rotateFootprint as rotateFootprintImpl,
  moveFootprint as moveFootprintImpl,
  groupMove as groupMoveImpl,
  type AddFootprintOptions,
  type RotateFootprintOptions,
  type MoveFootprintOptions,
  type GroupMoveOptions,
} from './circuitJsonPatch.js'

// ---------------------------------------------------------------------------
// Test-only element shape + casting wrappers
// ---------------------------------------------------------------------------
//
// circuitJsonPatch.ts's exported functions are honestly typed against real
// Circuit JSON (`CircuitJson`/`CircuitElement` from `@/types`), which is a
// schema-complete union (see that file's header comment on the impedance
// mismatch). This suite's fixtures — mirroring the pre-migration JS
// literals — are intentionally minimal test doubles that only carry the
// fields the functions under test actually read/write, not full schema
// compliance. Rather than weaken the module's real exported types to fit
// loose test literals, the cast lives here at the test boundary.
...
function callAddFootprint(circuitJson: unknown, opts?: AddFootprintOptions): TestElement[] {
  return addFootprintImpl(circuitJson as unknown as CircuitJson, opts) as unknown as TestElement[]
}
function callRotateFootprint(circuitJson: unknown, opts?: RotateFootprintOptions): TestElement[] {
  return rotateFootprintImpl(circuitJson as unknown as CircuitJson, opts) as unknown as TestElement[]
}
function callMoveFootprint(circuitJson: unknown, opts?: MoveFootprintOptions): TestElement[] {
  return moveFootprintImpl(circuitJson as unknown as CircuitJson, opts) as unknown as TestElement[]
}
function callGroupMove(circuitJson: unknown, opts?: GroupMoveOptions): TestElement[] {
  return groupMoveImpl(circuitJson as unknown as CircuitJson, opts) as unknown as TestElement[]
}
...
function makeBoard(): TestElement[] {
  return [{ type: 'pcb_board', width: 50, height: 50, thickness: 1.6 }]
}

function makeComponent(id: string, cx: number, cy: number, rotation = 0): TestElement {
  return {
    type: 'pcb_component',
    pcb_component_id: id,
    center: { x: cx, y: cy },
...
function makePad(padId: string, componentId: string, x: number, y: number): TestElement {
  return {
    type: 'pcb_smtpad',
    pcb_smtpad_id: padId,
...
function singleComponentCircuit(): TestElement[] {
  return [
    makeBoard(),
...
describe('addFootprint', () => {
  it('throws on non-array input', () => {
    expect(() => callAddFootprint(null, {})).toThrow(TypeError)
    expect(() => callAddFootprint('bad', {})).toThrow(TypeError)
  })

  it('returns a new array (immutable)', () => {
    const base = makeBoard()
    const result = callAddFootprint(base, { footprintFn: 'res0402', x: 0, y: 0 })
    expect(result).not.toBe(base)
    expect(base).toHaveLength(1) // original unmodified
  })

  it('adds a pcb_component entry', () => {
    const result = callAddFootprint(makeBoard(), { footprintFn: 'res0402', x: 10, y: 20 })
  ...
  it('adds at least one pad entry for res footprint', () => {
    const result = callAddFootprint(makeBoard(), { footprintFn: 'res0402', x: 0, y: 0 })
  ...
  it('uses supplied refdes as the component name', () => {
    const result = callAddFootprint(makeBoard(), { footprintFn: 'res0402', refdes: 'R42', x: 0, y: 0 })
    const comp = result.find((el) => el.type === 'pcb_component')
  ...
  it('shifts pads to the requested position', () => {
    const px = 15
    const py = 20
    const result = callAddFootprint(makeBoard(), { footprintFn: 'res0402', x: px, y: py })
  ...
  it('applies initial rotation to pads', () => {
    const base = makeBoard()
    const r0 = callAddFootprint(base, { footprintFn: 'res0402', x: 0, y: 0, rotation: 0 })
    const r90 = callAddFootprint(base, { footprintFn: 'res0402', x: 0, y: 0, rotation: 90 })
  ...
  it('stores the initial rotation on the pcb_component', () => {
    const result = callAddFootprint(makeBoard(), { footprintFn: 'res0402', rotation: 45 })
    const comp = result.find((el) => el.type === 'pcb_component')
    expect(comp.rotation).toBe(45)
  })

  it('handles dip footprint with full specifier dip8', () => {
    const result = callAddFootprint(makeBoard(), {
      footprintFn: 'dip8',
      x: 0,
      y: 0,
    })
  ...
  it('defaults passive (bare res) to 0402 size', () => {
    const result = callAddFootprint(makeBoard(), { footprintFn: 'res', x: 0, y: 0 })
    const comp = result.find((el) => el.type === 'pcb_component')
    expect(comp).toBeDefined()
    const pads = result.filter((el) => el.type === 'pcb_smtpad')
    expect(pads.length).toBeGreaterThan(0)
  ...
  it('returns a copy when footprintFn is unknown', () => {
    const base = makeBoard()
    const result = callAddFootprint(base, { footprintFn: 'not_a_real_footprint' })
  ...
  it('preserves existing elements unchanged', () => {
    ...
    const result = callAddFootprint(base, { footprintFn: 'cap0402', x: 30, y: 30 })
...
describe('rotateFootprint', () => {
  it('throws on non-array input', () => {
    expect(() => callRotateFootprint(null, { pcb_component_id: 'x', angleDeg: 0 })).toThrow(TypeError)
  })

  it('throws when pcb_component_id is missing', () => {
    expect(() => callRotateFootprint([], { angleDeg: 90 })).toThrow()
  })

  it('throws on non-finite angleDeg', () => {
    const c = singleComponentCircuit()
    expect(() => callRotateFootprint(c, { pcb_component_id: 'comp_a', angleDeg: NaN })).toThrow(TypeError)
    expect(() => callRotateFootprint(c, { pcb_component_id: 'comp_a', angleDeg: Infinity })).toThrow(TypeError)
  })

  it('returns a new array (immutable)', () => {
    const base = singleComponentCircuit()
    const result = callRotateFootprint(base, { pcb_component_id: 'comp_a', angleDeg: 90 })
    expect(result).not.toBe(base)
  ...
  it('accumulates rotation on the component', () => {
    ...
    c = callRotateFootprint(c, { pcb_component_id: 'comp_a', angleDeg: 30 })
    c = callRotateFootprint(c, { pcb_component_id: 'comp_a', angleDeg: 60 })
  ...
  it('returns a copy unchanged when component id does not exist', () => {
    const base = singleComponentCircuit()
    const result = callRotateFootprint(base, { pcb_component_id: 'nonexistent', angleDeg: 45 })
    expect(result).toHaveLength(base.length)
  })
...
describe('moveFootprint', () => {
  it('throws on non-array input', () => {
    expect(() => callMoveFootprint('bad', { pcb_component_id: 'x', x: 0, y: 0 })).toThrow(TypeError)
  })

  it('throws when pcb_component_id is missing', () => {
    expect(() => callMoveFootprint([], { x: 1, y: 1 })).toThrow()
  })

  it('throws on non-finite coordinates', () => {
    const c = singleComponentCircuit()
    expect(() => callMoveFootprint(c, { pcb_component_id: 'comp_a', x: NaN, y: 0 })).toThrow(TypeError)
    expect(() => callMoveFootprint(c, { pcb_component_id: 'comp_a', x: 0, y: Infinity })).toThrow(TypeError)
  })

  it('returns a new array (immutable)', () => {
    const base = singleComponentCircuit()
    const result = callMoveFootprint(base, { pcb_component_id: 'comp_a', x: 10, y: 10 })
    expect(result).not.toBe(base)
  ...
  it('updates component centre to the new absolute position', () => {
    const result = callMoveFootprint(singleComponentCircuit(), {
  ...
  it('returns copy unchanged when component id does not exist', () => {
    const base = singleComponentCircuit()
    const result = callMoveFootprint(base, { pcb_component_id: 'ghost', x: 0, y: 0 })
    expect(result).toHaveLength(base.length)
  })

  it('two successive moves are equivalent to one direct move', () => {
    let c = singleComponentCircuit()
    c = callMoveFootprint(c, { pcb_component_id: 'comp_a', x: 20, y: 10 })
    c = callMoveFootprint(c, { pcb_component_id: 'comp_a', x: 35, y: 25 })
    const direct = callMoveFootprint(singleComponentCircuit(), {
      pcb_component_id: 'comp_a',
      x: 35,
      y: 25,
    })
...
describe('groupMove', () => {
  it('throws on non-array circuitJson', () => {
    expect(() => callGroupMove(null, { pcb_component_ids: [], dx: 0, dy: 0 })).toThrow(TypeError)
  })

  it('throws on non-array pcb_component_ids', () => {
    expect(() => callGroupMove([], { pcb_component_ids: 'bad' as unknown as string[], dx: 0, dy: 0 })).toThrow(TypeError)
  })

  it('throws on non-finite delta', () => {
    const c = singleComponentCircuit()
    expect(() =>
      callGroupMove(c, { pcb_component_ids: ['comp_a'], dx: NaN, dy: 0 })
    ).toThrow(TypeError)
  })

  it('returns a new array (immutable)', () => {
    const base = singleComponentCircuit()
    const result = callGroupMove(base, { pcb_component_ids: ['comp_a'], dx: 5, dy: 0 })
    expect(result).not.toBe(base)
    const origComp = base.find((el) => el.pcb_component_id === 'comp_a' && el.type === 'pcb_component')
    expect(origComp.center.x).toBe(5)
  })
  ...
  it('returns a copy unchanged for empty ids list', () => {
    const base = singleComponentCircuit()
    const result = callGroupMove(base, { pcb_component_ids: [], dx: 10, dy: 10 })
    expect(result).toHaveLength(base.length)
    const comp = result.find((el) => el.pcb_component_id === 'comp_a' && el.type === 'pcb_component')
  ...
  it('moves all listed components by the delta', () => {
    ...
    const result = callGroupMove(base, {
  ...
  it('shifts pads along with their parent', () => {
    const result = callGroupMove(singleComponentCircuit(), {
