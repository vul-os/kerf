...
3-axis operations can reference a saved `.tool` file from the project's tool
database (T7).  Pass `tool_id` to `cam_run` to load the matching tool and let
its feeds/speeds drive the G-code output.

### `cam_run` additional fields

```
tool_id        Tool DB id to load (e.g. "T1").  Optional.
               When set: the tool's feed_rate_mm_min, plunge_rate_mm_min,
               and spindle_rpm_min are used as defaults for the 3-axis post.
               Caller-supplied feed_rate / spindle_speed still override.
               Returns TOOL_NOT_FOUND if the id does not exist in the project.

post_processor 3-axis post-processor: "linuxcnc" (default) | "grbl" | "mach3" | "fanuc"
               Ignored for 5-axis operations (use post_processor_5x there).
```

### Post-processor dialect summary

| Post       | Tool-change      | Comments      | Tape markers | Notes |
|------------|-----------------|---------------|--------------|-------|
| linuxcnc   | `M6 T<n>`       | `; …`         | `%` / `%`    | G43 TLO |
| grbl       | `(M6 T<n>)`     | `; …`         | `%` / `%`    | No tool changer — comment only |
| mach3      | `T<n> M6`       | `(…)`         | none         | G43 TLO |
| fanuc      | `M6 T<n>`       | `(…)`         | none         | N-line sequence numbers |

**GRBL M6 rationale:** GRBL has no built-in tool changer.  A bare `M6` would
alarm or halt the machine depending on firmware version.  Kerf emits `(M6 T<n>)`
as a comment following the established convention used by Carbide Create, CNC.js,
and Easel — the operator sees which tool to load on the setup sheet, then presses
Cycle Start.

### Example: pocket with tool DB
...
The resulting G-code header (LinuxCNC):

```
%
; Generated by Kerf CAM — LinuxCNC 3-axis post
; tool: T1 — 6mm carbide flat-end, ø6 mm, 2-flute, carbide
G90 G94 G17 G21
G54
M6 T1
G43 H1
...
S8000 M3
M8
G0 X0.000 Y0.000 Z1.000 F5000
G1 Z-1.000 F200
G1 X0.000 Y0.000 Z-1.000 F750
...
G0 Z50.000 F5000
G49
M9
